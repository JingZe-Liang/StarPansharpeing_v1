...
class MultiScaleBSQ(Module):
    ...
    def forward(self, x, scale_schedule=None, mask=None, return_all_codes=False, return_residual_norm_per_scale=False):
        if x.ndim == 4:
            x = x.unsqueeze(2)
        B, C, T, H, W = x.size()
            ...
            for si, (pt, ph, pw) in enumerate(scale_schedule):
                ...
                if self.training and self.use_stochastic_depth and random.random() < self.drop_rate:
                    if (si == 0 and self.keep_first_quant) or (si == scale_num - 1 and self.keep_last_quant):
                        (quantized, indices, bit_indices, loss), breakdown = self.lfq(
                            interpolate_residual, return_loss_breakdown=True
                        )
                        all_entropies.append(torch.stack([breakdown.per_sample_entropy, breakdown.batch_entropy]))
                    else:
                ...
                if self.remove_residual_detach:
                    residual = residual - quantized_up
                else:
                    residual = residual - quantized_up.detach()
                quantized_out = quantized_out + quantized_up
        ...
        all_entropies = torch.stack(all_entropies, dim=-1) if len(all_entropies) > 0 else self.zero
...
if __name__ == "__main__":
    """
    python -m src.stage1.discretization.InfinityCC.infinity.models.bsq_vae.multiscale_bsq
    """
    __test_bsq()
