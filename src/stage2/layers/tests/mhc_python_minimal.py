...
def main() -> int:
    ...
    init_hc, expand_stream, reduce_stream = mhc_mod.get_init_and_expand_reduce_stream_functions(streams)
    ...
    assert logits.shape == (batch, block_size, vocab_size)

    loss = logits.float().mean()
    loss.backward()

    print("TinyGPTLikeModel + HyperConnections(mHC python) 跑通")
