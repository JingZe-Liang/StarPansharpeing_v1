"""
Block-sparse softmax cross-attention Triton kernels.

This is adapted from `src/stage2/layers/SLA/kernel.py` but supports Lq != Lk.
Q has shape (B, H, Lq, D) while K/V have shape (B, H, Lk, D).
"""
...
def _xattn_bwd_preprocess(
    ...
    o_s = tl.load(OS + offs_m[:, None] * D + offs_d[None, :], mask=offs_m[:, None] < LQ)
    do_s = tl.load(DOS + offs_m[:, None] * D + offs_d[None, :], mask=offs_m[:, None] < LQ)
...
class _cross_attention(torch.autograd.Function):
    @staticmethod
