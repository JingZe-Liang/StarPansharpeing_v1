...
class SEN12MSCRDataset:
    def __init__(self, base_dir):
        ...
        if not os.path.exists(self.base_dir):
            raise Exception("The specified base_dir for SEN12MS-CR dataset does not exist")
    ...
    def get_scene_ids(self, season):
        ...
        # Use s1 folder to find scene IDs, assuming scenes are consistent across sensors
        path = os.path.join(self.base_dir, season, f"{season}_s1")

        if not os.path.exists(path):
            raise NameError("Could not find season folder {} in base directory {}".format(path, self.base_dir))

        scene_list = [os.path.basename(s) for s in glob(os.path.join(path, "s1_*"))]
    ...
    def get_patch_ids(self, season, scene_id):
        season = Seasons(season).value
        path = os.path.join(self.base_dir, season, f"{season}_s1", f"s1_{scene_id}")
    ...
    def get_patch(self, season, scene_id, patch_id, bands):
        ...
        patch_path = os.path.join(self.base_dir, season, f"{season}_{sensor}", scene, filename)
    ...
    def get_s1s2s2cloudy_triplet(
        self, season, scene_id, patch_id, s1_bands=S1Bands.ALL, s2_bands=S2Bands.ALL, s2cloudy_bands=S2Bands.ALL
    ):
...
if __name__ == "__main__":
    ...
    sen12mscr = SEN12MSCRDataset("/home/user/zihancao/Project/hyperspectral-1d-tokenizer/data/SEN12MS-CR")
    ...
    SCENE_ID = 8
    s2_rgb_patch, bounds = sen12mscr.get_patch(Seasons.SPRING, SCENE_ID, spring_ids[SCENE_ID][0], bands=S2Bands.RGB)
    ...
    s1, s2, s2cloudy, _ = sen12mscr.get_triplets(
        Seasons.SPRING, 106, s1_bands=S1Bands.ALL, s2_bands=S2Bands.ALL, s2cloudy_bands=S2Bands.ALL
    )
