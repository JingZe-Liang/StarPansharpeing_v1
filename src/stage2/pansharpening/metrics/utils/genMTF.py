...
def genMTF_torch(ratio, sensor, nbands):
    ...
    if sensor == "QB":
        GNyq = torch.tensor([0.34, 0.32, 0.30, 0.22], dtype=torch.float32)  # Band Order: B,G,R,NIR
    elif sensor == "IKONOS":
        GNyq = torch.tensor([0.26, 0.28, 0.29, 0.28], dtype=torch.float32)  # Band Order: B,G,R,NIR
    elif sensor in ["GeoEye1", "WV4"]:
        GNyq = torch.tensor([0.23, 0.23, 0.23, 0.23], dtype=torch.float32)  # Band Order: B,G,R,NIR
    elif sensor == "WV2":
