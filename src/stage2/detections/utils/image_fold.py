...
class WindowSlider(nn.Module):
    def __init__(self, wsize=15, wstride=5):
        super(WindowSlider, self).__init__()
        self.ksize = wsize
        self.stride = wstride
...
class WindowMerger(nn.Module):
    def __init__(self, wsize=15, wstride=5):
        super(WindowMerger, self).__init__()
        self.ksize = wsize
        self.stride = wstride
...
class CMMBlockSearch(nn.Module):
    def __init__(self, x_ori, wsize=15, wstride=5):
        super(CMMBlockSearch, self).__init__()
        self.ksize = wsize
        self.stride = wstride
        self.dis = torch.nn.PairwiseDistance(p=2, keepdim=True)

        self.win_slider = WindowSlider(wsize=self.ksize, wstride=self.stride)
        block_query, _ = self.win_slider.extract_image_blocks(
            x_ori, ksizes=[self.ksize, self.ksize], strides=[self.stride, self.stride]
        )
    ...
    def forward(self, x, match_vec, idx):
        block_key, _ = self.win_slider.extract_image_blocks(
