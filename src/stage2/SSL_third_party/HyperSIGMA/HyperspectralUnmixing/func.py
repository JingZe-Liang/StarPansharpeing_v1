...
def get_args(Dataset, seed, use_checkpoint=True, mode="Spat_Spec_Pretraining"):
    ...
    parser.add_argument(
        "--epochs", default=200, type=int, metavar="N", help="defalut: number of total epochs to run"
    )  # 320, 200, 300
    ...
    parser.add_argument("--data_file", default=data_file, type=str, help="path filename of the trained model")
...
def get_train_patch_seed_i(img_size, img, train_number, batch_size, seed_i):
    ...
    img_train = get_train_data(mirror_img, band, total_pos_train, patch=img_size, band_patch=1)
    # load data
