...
def train_model(args, save_PATH):
    ...
    train_transform = transform.Compose([transform.RandomHorizontalFlip(p=0.5), transform.RandomVerticalFlip(0.5)])
    ...
    model_dict["decoder.decoder.weight"][:, :, args.kernel // 2, args.kernel // 2] = torch.from_numpy(init_em).float()
    ...
    for _epoch in range(0, args.epochs):
        ...
        if _epoch % 20 == 0:
            print(
                "epoch [{}/{}],train loss:{:.4f},loss_sad:{:.4f},loss_ab:{:.4f},loss_tv:{:.4f}".format(
                    _epoch + 1, args.epochs, l, l_sad, l_ab, l_tv
                )
            )
...
def test_model(args, model_save_PATH, result_save_file):
    ...
    SAD_ordered, endmember_sordered, abundance_sordered = plotEndmembersAndGT(endmembers.T, endmember_GT, abundances)
    ...
    sio.savemat(
        result_save_file,
        {
            "decoder_weight": endmembers,
            "endmembers": endmember_sordered,
...
def run(Dataset_name, seed, mode="Spat_Spec_Pretraining", use_checkpoint=True):
    ...
    t0 = time.time()
    args = get_args(Dataset_name, seed, use_checkpoint=use_checkpoint, mode=mode)
    if args.use_checkpoint:
