...
def main():
    ...
    train_parser.add_argument(
        "--cuda", type=int, required=False, default=1, help="set it to 1 for running on GPU, 0 for CPU"
    )
    ...
    train_parser.add_argument(
        "--dataset_name", type=str, default="pavia", help="dataset_name, default set to dataset_name"
    )
    train_parser.add_argument(
        "--model_title", type=str, default="HSIformer36_0.5_0.1", help="model_title, default set to model_title"
    )
    train_parser.add_argument(
        "--pretrain_path",
        type=str,
        default="/mnt/code/users/yuchunmiao/SST-master/pre_train/spat-vit-base-ultra-checkpoint-1599.pth",
        help="pretrain_path",
    )
    ...
    test_parser.add_argument(
        "--dataset_name", type=str, default="Chikusei", help="dataset_name, default set to dataset_name"
    )
    test_parser.add_argument(
        "--model_title", type=str, default="vit_series4_final", help="model_title, default set to model_title"
    )
    ...
    test_parser.add_argument(
        "--weight_path", type=str, default="Chikusei", help="dataset_name, default set to dataset_name"
    )
    test_parser.add_argument(
        "--pretrain_path",
        type=str,
        default="/mnt/code/users/yuchunmiao/SST-master/pre_train/checkpoint-1599.pth",
        help="pretrain_path",
    )
    ...
    for e in range(start_epoch, args.epochs):
            ...
            if (iteration + log_interval) % log_interval == 0:
                ...
                print(
                    "===> {} \tEpoch[{}]({}/{}): Loss: {:.6f}".format(
                        time.ctime(), e + 1, iteration + 1, len(train_loader) - 1, loss.item()
                    )
                )
        ...
        with torch.no_grad():
            ...
            for i, (ms, lms, gt) in enumerate(test_loader):
                ...
                for i in range(num_blocks):
                    for j in range(num_blocks):
                        ...
                        y[
                            :,
                            :,
                            start_idx_i * args.n_scale : end_idx_i * args.n_scale,
                            start_idx_j * args.n_scale : end_idx_j * args.n_scale,
                        ] = net(
                            ms[:, :, start_idx_i:end_idx_i, start_idx_j:end_idx_j],
                            lms[
                                :,
                                :,
                                start_idx_i * args.n_scale : end_idx_i * args.n_scale,
                ...
                psnr_value = compare_mpsnr(gt, y, data_range=1.0)
...
def test(args):
    ...
    with torch.no_grad():
        ...
        for i, (ms, lms, gt) in enumerate(test_loader):
            ...
            y = y[: gt.shape[0], : gt.shape[1], :]
            if i == 0:
                indices = quality_assessment(gt, y, data_range=1.0, ratio=4)
            else:
                indices = sum_dict(indices, quality_assessment(gt, y, data_range=1.0, ratio=4))
            output.append(y)
            test_number += 1
