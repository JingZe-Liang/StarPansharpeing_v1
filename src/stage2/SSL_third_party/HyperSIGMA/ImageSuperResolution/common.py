...
def group_conv(in_channels, out_channels, kernel_size, groups, stride=1, bias=True):
    return nn.Conv2d(
        in_channels, out_channels, kernel_size, padding=(kernel_size // 2), stride=stride, bias=bias, groups=groups
    )
...
class conv_sep(nn.Module):
    def __init__(self, in_channels, out_channels, ks, stride=1, dilation=1):
        ...
        self.deepwise = default_conv(
            in_channels, in_channels, kernel_size=ks, stride=stride, dilation=dilation, groups=in_channels
        )
