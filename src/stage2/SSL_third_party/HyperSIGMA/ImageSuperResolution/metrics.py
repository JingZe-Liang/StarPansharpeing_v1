...
def compare_mpsnr_test(x_true, x_pred, data_range):
    ...
    channels = x_true.shape[2]
    total_psnr = [
        compare_psnr(im_true=x_true[:, :, k], im_test=x_pred[:, :, k], data_range=data_range) for k in range(channels)
    ]
...
def compare_mssim(x_true, x_pred, data_range, multidimension):
    ...
    """
    mssim = [
        compare_ssim(im1=x_true[:, :, i], im2=x_pred[:, :, i], data_range=data_range, multidimension=multidimension)
...
def compare_sid(x_true, x_pred):
    ...
    for i in range(N):
        err[i] = abs(
            np.sum(x_pred[:, :, i] * np.log10((x_pred[:, :, i] + 1e-3) / (x_true[:, :, i] + 1e-3)))
