...
def default_conv(in_channels, out_channels, kernel_size, stride=1, bias=True, dilation=1, groups=1):
    if dilation == 1:
        ...
        return nn.Conv2d(in_channels, out_channels, kernel_size, padding=2, bias=bias, dilation=dilation, groups=groups)
...
class SampleAttention(nn.Module):
    def __init__(
        ...
        self.sampling_offsets = nn.Linear(
            all_head_dim, self.num_heads * n_points * 2
        )  # 通过n point设置每个level采样几个点
    ...
    def forward(self, x, H, W, deform_inputs):
        ...
        sampling_offsets = (
            self.sampling_offsets(q).reshape(B, N, self.num_heads, self.n_points, 2).transpose(1, 2)
        )  # B, N, H*P*2 -> B, H, N, P, 2
        ...
        k = (
            k.flatten(0, 1)
            .transpose(1, 2)
            .reshape(B * self.num_heads, L // self.num_heads, input_spatial_shapes[0], input_spatial_shapes[1])
        )
        v = (
            v.flatten(0, 1)
            .transpose(1, 2)
...
class SpatViT(nn.Module):
    ...
    def __init__(
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed_add = PatchEmbed(img_size=img_size, patch_size=1, in_chans=self.embed_dim, embed_dim=180)
        ...
        if self.pos_embed_add is not None:
            trunc_normal_(self.pos_embed_add, std=0.02)
