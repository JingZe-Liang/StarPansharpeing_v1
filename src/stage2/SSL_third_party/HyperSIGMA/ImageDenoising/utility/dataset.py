...
def addNoiseComplex(srcdir, dstdir):
    ...
    for noise_name, noise_model in zip(names, noise_models):
        ...
        for filename in os.listdir(srcdir):
            ...
            savemat(
                os.path.join(dstdir_noise, filename),
                {"gt": loadmat(os.path.join(srcdir, filename))["data"], "input": noisyhsi},
            )
...
if __name__ == "__main__":
    srcdir = (
        "/mnt/code/users/yuchunmiao/hypersigma-master/data/Hyperspectral_Project/WDC/test"  # file path of original file
    )
