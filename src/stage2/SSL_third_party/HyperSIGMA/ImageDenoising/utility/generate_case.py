...
class AddNoiseNoniid_v2(object):
    ...
    def __call__(self, img):
        bwsigmas = np.reshape(
            (np.random.rand(img.shape[0]) * (self.max_sigma - self.min_sigma) + self.min_sigma), (-1, 1, 1)
        )
...
class AddNoiseCase3(AddNoiseMixed):
    def __init__(self):
        self.noise_bank = [_AddNoiseDeadline(0.05, 0.15), _AddNoiseImpulse([0.1, 0.3, 0.5, 0.7])]
        self.num_bands = [1 / 3, 1 / 3, 1 / 3]


class AddNoiseCase4(AddNoiseMixed):
    def __init__(self):
        self.noise_bank = [_AddNoiseStripe(0.05, 0.15), _AddNoiseImpulse([0.1, 0.3, 0.5, 0.7])]
        self.num_bands = [1 / 3, 1 / 3, 1 / 3]
...
class DataLoaderVal(Dataset):
    def __init__(self, data_dir, ratio=50, target_transform=None):
        ...
        self.noisy_filenames = [
            os.path.join(data_dir, "input{}".format(ratio), x) for x in noisy_files if is_tif_file(x)
        ]

        self.clean = [
            torch.from_numpy(np.float32(load_tif_img(self.clean_filenames[index])))
            for index in range(len(self.clean_filenames))
        ]
        self.noisy = [
            torch.from_numpy(np.float32(load_tif_img(self.noisy_filenames[index])))
            for index in range(len(self.noisy_filenames))
        ]
...
def get_train_valid_loader(
    ...
    train_dataset = TransformDataset(SplitDataset(dataset, partitions, initial_partition="Train"), train_transform)

    valid_dataset = TransformDataset(SplitDataset(dataset, partitions, initial_partition="Valid"), valid_transform)

    train_loader = DataLoader(
        train_dataset, batch_size=batch_size[0], shuffle=True, num_workers=num_workers, pin_memory=pin_memory
    )

    valid_loader = DataLoader(
        valid_dataset, batch_size=batch_size[1], shuffle=False, num_workers=num_workers, pin_memory=pin_memory
    )
    ...
    for sigma in s_sigma:
        ...
        for filename in os.listdir(srcdir):
            ...
            savemat(
                ...
                {"gt": srchsi.transpose(1, 2, 0), "sigma": n_sigma, "input": noisyhsi.transpose(1, 2, 0)},
            )
    ...
    for noise_name, noise_model in zip(names, noise_models):
        ...
        for filename in os.listdir(srcdir):
            ...
            savemat(
                os.path.join(dstdir_noise, filename),
                {
                    "gt": loadmat(os.path.join(srcdir, filename))["data"][:, 0:192, 0:192],
                    "input": noisyhsi[:, 0:192, 0:192],
                },
            )
