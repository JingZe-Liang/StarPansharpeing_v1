...
def train_options(parser):
    ...
    parser.add_argument(
        ...
        choices=["l1", "l2", "smooth_l1", "ssim", "l2_ssim", "l2_sam", "cons", "cons_l2", "char"],
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="/mnt/code/users/yuchunmiao/hypersigma-master/data/Hyperspectral_Project/WDC/wdc_64.db",
    )
    parser.add_argument(
        "--pretrain", type=str, default="/mnt/code/users/yuchunmiao/hypersigma-master/pre_train/checkpoint-400.pth"
    )

    parser.add_argument(
        "--init", type=str, default="kn", help="which init scheme to choose.", choices=["kn", "ku", "xn", "xu", "edsr"]
    )
    parser.add_argument("--no-cuda", action="store_true", help="disable cuda?")
    parser.add_argument("--from_scratch", action="store_true", help="disable cuda?")
    parser.add_argument(
        "--pretrain_path", type=str, default="/mnt/code/users/yuchunmiao/hypersigma-master/pre_train/checkpoint-400.pth"
    )
    ...
    parser.add_argument("--dataroot", "-d", type=str, default="/data/HSI_Data/ICVL64_31.db", help="data root")
...
def make_dataset(opt, train_transform, target_transform, common_transform, batch_size=None, repeat=1):
    ...
    train_loader = DataLoader(
        train_dataset,
        batch_size=batch_size or opt.batchSize,
        shuffle=True,
...
class Engine(object):
    ...
    def train(self, train_loader, val):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            if not self.opt.no_log:
                self.writer.add_scalar(join(self.prefix, "train_psnr"), avg_psnr, self.iteration)
            ...
            progress_bar(
                batch_idx,
                len(train_loader),
                "AvgLoss: %.4e | Loss: %.4e | Norm: %.4e | Psnr: %4e" % (avg_loss, loss_data, total_norm, psnr),
            )
    ...
    def test(self, valid_loader, filen):
        ...
        ERGAS = []
        PSNR = []
        filenames = [fn for fn in os.listdir(filen) if fn.endswith(".mat")]
        ...
        with torch.no_grad():
            ...
            for batch_idx, (inputs, targets) in enumerate(valid_loader):
                ...
                progress_bar(
                    batch_idx,
                    len(valid_loader),
                    "Loss: %.4e | PSNR: %.4f | AVGPSNR: %.4f " % (avg_loss, psnr, avg_psnr),
                )
                ...
                for k in range(c):
                    ssim.append(
                        (2 * np.mean(result[k]) * np.mean(img[k]) + k1**2)
                ...
                temp = (
                    (np.sum(result * img, 0) + np.spacing(1))
        ...
        save_dir = (
            f"/mnt/code/users/yuchunmiao/hypersigma-master/data/Hyperspectral_Project/WDC/results/{self.opt.arch}/"
        )
        ...
        scio.savemat(
            save_target,
            {
                "gt": all_gt.transpose(1, 2, 0),
                "input": all_input.transpose(1, 2, 0),
                "output": all_output.transpose(1, 2, 0),
            },
        )
    ...
    def validate(self, valid_loader, name, patch_size=64, inpainting=False):
        ...
        if not self.opt.no_log:
            ...
            self.writer.add_scalar(join(self.prefix, name, "val_sam_epoch"), avg_sam, self.epoch)
        print(avg_psnr, avg_loss, avg_sam)
        self.psnr = avg_psnr
