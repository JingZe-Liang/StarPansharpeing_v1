...
class Trainer(object):
    def __init__(self, args):
        ...
        Spat_pernet = torch.load(
            "/home/songjian/project/HSIFM/pretrained_weight/HSI_spatial_checkpoint-1600.pth",
        ...
        Spec_pernet = torch.load(
            "/home/songjian/project/HSIFM/pretrained_weight/spec-vit-base-ultra-checkpoint-1599.pth",
            map_location=torch.device("cpu"),
        )
        ...
        self.optim = optim.AdamW(self.deep_model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num)):
            ...
            if (itera + 1) % 10 == 0:
                print(f"iter is {itera + 1},  change detection loss is {bcd_loss}")
                if (itera + 1) % 100 == 0:
                    self.deep_model.eval()
                    rec, pre, oa, f1_score, iou, kc = self.validation()
                    if kc > best_kc:
                        torch.save(
                            self.deep_model.state_dict(), os.path.join(self.model_save_path, f"{itera + 1}_model.pth")
                        )
    ...
    def validation(self):
        ...
        dataset = OSCDDatset13Bands(
            dataset_path=dataset_path, data_list=data_name_list, crop_size=512, max_iters=None, type="test"
        )
        ...
        print(
            f"Racall rate is {rec}, Precision rate is {pre}, OA is {oa}, "
...
def main():
    ...
    parser.add_argument("--dataset_path", type=str, default="/home/songjian/project/HSIFM/dataset/OSCD/original_data")
    parser.add_argument("--type", type=str, default="train")
    parser.add_argument(
        "--train_data_list_path", type=str, default="/home/songjian/project/HSIFM/dataset/OSCD/original_data/train.txt"
    )
