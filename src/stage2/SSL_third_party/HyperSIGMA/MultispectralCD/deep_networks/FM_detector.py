...
class MSCDNet(nn.Module):
    def __init__(self, use_checkpoint=True):
        ...
        self.spat_encoder = ALter_SpatViT(
            img_size=128,
            in_chans=100,
            ...
            interval=3,
            pretrained="/home/songjian/project/HSIFM/pretrained_weight/HSI_spatial_checkpoint-1600.pth",
        )
        NUM_TOKENS = 100
        ...
        self.fuse_layer_4 = nn.Sequential(
            nn.Conv2d(kernel_size=1, in_channels=768 * 2, out_channels=128), nn.BatchNorm2d(128), nn.ReLU()
        )
