...
def lovasz_softmax_flat(probas, labels, classes="present"):
    ...
    if probas.numel() == 0:
        # only void pixels, the gradients should be 0
        return 0.0  # current probas.shape=torch.size([0,4]) and (probas*0.).shape is also torch.size([0,4]),a tensor loss cannot be backforward
    if len(probas.shape) == 1:
        probas.unsqueeze(
            0
        )  # only one pixel match the interest classes, probas.shape=torch.size([4]), we need it to be torch.size([1,4]) for the subsequent calculation
    C = probas.size(1)
    losses = []
