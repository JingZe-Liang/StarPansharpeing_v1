...
def get_args(dataset, seed_i, use_checkpoint=True, mode="Spat_Spec_Pretraining"):
    ...
    parser.add_argument(
        "--dataset", choices=["BayArea", "Barbara", "Farmland", "Hermiston"], default=dataset, help="dataset to use"
    )
    ...
    if mode == "Spat_Pretraining":
        ...
        if use_checkpoint:
            if os.path.isfile(path + "spat-vit-base-ultra-checkpoint-1599.pth"):
                ...
                print(
                    "Please download the pretrained model checkpoint firstly at "
        ...
        parser.add_argument("--pretrain_path", type=str, default=pretrain_path, help="")
        ...
        parser.add_argument("--pretrain_Spatpath", type=str, default=pretrain_Spatpath, help="")
        parser.add_argument("--pretrain_Specpath", type=str, default=pretrain_Specpath, help="")
        parser.add_argument("--interval", type=int, default=3, help="")
        parser.add_argument("--embed_dim", type=int, default=768, help="")
    ...
    print("train_number:", args.train_number, " | batch_size:", args.batch_size, " | epochs:", args.epochs)
...
def get_Traindata(args):
    ...
    x_train_t1, _ = get_train_and_test_data(
        mirror_image_t1, band, total_pos_train, total_pos_test, patch=args.patch_size, band_patch=args.band_patches
    )
    x_train_t2, _ = get_train_and_test_data(
        mirror_image_t2, band, total_pos_train, total_pos_test, patch=args.patch_size, band_patch=args.band_patches
    )
...
def get_Testdata(args):
    ...
    _, x_test_t1 = get_train_and_test_data(
        mirror_image_t1, band, total_pos_train, total_pos_test, patch=args.patch_size, band_patch=args.band_patches
    )
    _, x_test_t2 = get_train_and_test_data(
        mirror_image_t2, band, total_pos_train, total_pos_test, patch=args.patch_size, band_patch=args.band_patches
    )
...
def gain_neighborhood_band(x_train, band, band_patch, patch=5):
    ...
    for i in range(nn):
        if pp > 0:
            x_train_band[:, i * patch * patch : (i + 1) * patch * patch, : i + 1] = x_train_reshape[
                :, :, band - i - 1 :
            ]
            x_train_band[:, i * patch * patch : (i + 1) * patch * patch, i + 1 :] = x_train_reshape[
                :, :, : band - i - 1
    ...
    for i in range(nn):
        if pp > 0:
            x_train_band[:, (nn + i + 1) * patch * patch : (nn + i + 2) * patch * patch, : band - i - 1] = (
                x_train_reshape[:, :, i + 1 :]
            )
            x_train_band[:, (nn + i + 1) * patch * patch : (nn + i + 2) * patch * patch, band - i - 1 :] = (
                x_train_reshape[:, :, : i + 1]
