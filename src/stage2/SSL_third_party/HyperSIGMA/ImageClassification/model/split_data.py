...
sys.path.append(r"E:\HSI_Classification\data_preprocess\Load")
...
def create_patches(X, y, window_size=5, remove_zero_labels=True):
    ...
    patchesData = np.zeros(
        (X.shape[0] * X.shape[1], window_size, window_size, X.shape[2]), dtype=np.float32
    )  # (21025, 5, 5, 30)
    ...
    for i in range(int(train.shape[0] / 2)):
        ...
        if num == 2:
            ...
            flipped_patch = scipy.ndimage.interpolation.rotate(
                patch, no, axes=(1, 0), reshape=False, output=None, order=3, mode="constant", cval=0.0, prefilter=False
            )
...
def save_preprocessed_data(
    path, data_all, data_label_all, train, train_label, val, val_label, test, test_label, save_data
):
