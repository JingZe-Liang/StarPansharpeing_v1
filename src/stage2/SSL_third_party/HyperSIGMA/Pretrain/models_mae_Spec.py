...
class SpecMAE(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_pos_embed = nn.Parameter(
            torch.zeros(1, NUM_TOKENS, decoder_embed_dim), requires_grad=False
        ...
        self.decoder_pred = nn.Linear(
            decoder_embed_dim, in_chans // NUM_TOKENS * (img_size**2), bias=True
        )  # decoder to patch
    ...
    def forward_decoder(self, x, ids_restore, mask):
        ...
        for blk in self.decoder_blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, int(x.shape[1] ** 0.5), int(x.shape[1] ** 0.5))
            else:
                x = blk(x, int(x.shape[1] ** 0.5), int(x.shape[1] ** 0.5))
