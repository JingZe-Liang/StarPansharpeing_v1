...
class HyperionDatasetChanSplit(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_vis_fill = np.pad(
            data_vis, ((0, fillBandNum - data_vis.shape[0]), (0, 0), (0, 0)), "constant", constant_values=(0, 0)
        )
        data_unvis_fill = np.pad(
            data_unvis, ((0, fillBandNum - data_unvis.shape[0]), (0, 0), (0, 0)), "constant", constant_values=(0, 0)
        )
        data_vis_mask_fill = np.pad(
            data_vis_mask,
            ((0, fillBandNum - data_vis_mask.shape[0]), (0, 0), (0, 0)),
            "constant",
            constant_values=(0, 0),
        )
        data_unvis_mask_fill = np.pad(
            data_unvis_mask,
            ((0, fillBandNum - data_unvis_mask.shape[0]), (0, 0), (0, 0)),
            "constant",
            constant_values=(0, 0),
        )  # 值为1的地方是padding的位置

        return (
            torch.tensor(np.array(data_vis_fill)).float(),
            torch.tensor(np.array(data_unvis_fill)).float(),
            torch.tensor(np.array(data_vis_mask_fill)).int(),
            torch.tensor(np.array(data_unvis_mask_fill)).int(),
        )
