...
def train(args, epoch, net, optimizer, scheduler, trn_loader, criterion):
    ...
    for idx, (X_data, y_target, ts_vector) in enumerate(trn_loader):
        ...
        if (idx + 1) % args.print_freq == 0:
            print(
                "Epoch: [{}/{}][{}/{}], Batch Loss {loss_meter.val:.4f}".format(
                    epoch + 1, args.epochs, idx + 1, len(trn_loader), loss_meter=loss_meter
                )
            )
...
def validation(args, epoch, net, val_loader):
    ...
    for idx, (X_data, y_target, ts_vector) in enumerate(val_loader):
        ...
        if (idx + 1) % args.print_freq == 0:
            print(
                "Epoch: [{}/{}][{}/{}], AUC {accuracy:.4f}.".format(
                    epoch + 1, args.epochs, idx + 1, len(val_loader), accuracy=AUC1_meter.val
                )
            )

    print("Validation epoch [{}/{}]: Avg AUC {:.4f}.".format(epoch + 1, args.epochs, AUC1_meter.avg))
...
if args.input_mode == "whole":
    ...
    Lx = np.linspace(
        0, LxEnd, int(np.ceil(LxEnd / float(input_size[1] - args.overlap_size))) + 1, endpoint=True
    ).astype("int")
    Ly = np.linspace(
        0, LyEnd, int(np.ceil(LyEnd / float(input_size[0] - args.overlap_size))) + 1, endpoint=True
    ).astype("int")
...
print(
    "{} image preparation Finished!, Data Number {}, Data size ({},{})".format(
        args.dataset, X_data.shape[0], X_data.shape[1], X_data.shape[2]
    )
)
...
for count in range(0, args.experiment_num):
    ...
    trn_loader = DataLoader(
        trn_dataset, batch_size=args.batch_size, num_workers=args.workers, shuffle=True, drop_last=True, pin_memory=True
    )
        ...
        for j in range(len(Ly)):
            for k in range(len(Lx)):
                ...
                if j == 0 and k == 0:
                    ...
                    y_tes_pred[
                        rStart + int(args.overlap_size / 2) : rEnd, cStart + int(args.overlap_size / 2) : cEnd
                    ] = part_pred[int(args.overlap_size / 2) :, int(args.overlap_size / 2) :]
                    y_tes_pred_map[
                        rStart + int(args.overlap_size / 2) : rEnd, cStart + int(args.overlap_size / 2) : cEnd
                    ] = part_pred_map[int(args.overlap_size / 2) :, int(args.overlap_size / 2) :]
