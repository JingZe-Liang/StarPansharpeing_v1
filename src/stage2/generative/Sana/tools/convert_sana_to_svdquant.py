...
def main(args):
    ...
    print(
        f"used mlp_ratio: {mlp_ratio}, hidden_size: {hidden_size}, mlp_hidden_features: {mlp_hidden_features}"
    )
    ...
    def handle_mismatched_shapes(key, checkpoint_param, current_shape):
        new_param = torch.zeros(
            current_shape, dtype=checkpoint_param.dtype, device=checkpoint_param.device
        )
        ...
        if (
            ...
            new_param[num_new_channels : num_new_channels + num_old_channels] = (
    ...
    for depth in range(layer_num):
        ...
        ff_inverted_bias_target_key = (
            f"transformer_blocks.{depth}.ff.conv_inverted.bias"
        )
        ...
        if inverted_bias_shape == ff_inverted_bias_param.shape:
            ...
            converted_state_dict[ff_inverted_bias_target_key] = (
                handle_mismatched_shapes(
