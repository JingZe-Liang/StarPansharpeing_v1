...
PngImagePlugin.MAX_TEXT_CHUNK = (
    100 * 1024 * 1024
)  # Increase maximum size for text chunks
...
def process_data(input_dir, output_tar_name="output.tar"):
    ...
    with tarfile.open(output_tar_name, "w") as tar:
        for item in os.listdir(input_dir):
            ...
            tar.add(
                item_path, arcname=item
            )  # arcname maintains the relative path of the file in the tar
...
if __name__ == "__main__":
    input_directory = input(
        "Please enter the directory path containing PNG and TXT files: "
    )
    output_tar_filename = (
        input("Please enter the name of the output TAR file (default is output.tar): ")
        or "output.tar"
    )
