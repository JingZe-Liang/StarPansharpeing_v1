...
def main(
    ...
    if pick_number != 2 ** int(np.log2(pick_number)) or number_of_files != 2 ** int(
        np.log2(number_of_files)
    ):
        print(
        ...
        print(
            f"warning: adjusted to 2^n, pick_number: {pick_number}, number_of_files: {number_of_files}"
        )
    ...
    for idx in range(start_idx, end_idx):
        ...
        new_metadata_path = osp.join(
            output_dir, base_dir, f"{idx:05d}", "metadata.jsonl"
        )
        ...
        if (
            ...
            and len(os.listdir(osp.join(output_dir, base_dir, f"{idx:05d}", "samples")))
            == 4
        ):
