...
def main() -> None:
    ...
    if (
        "SANA_SLURM_ACCOUNT" not in os.environ
        or "SANA_SLURM_PARTITION" not in os.environ
    ):
        raise ValueError(
            "`SANA_SLURM_ACCOUNT` and `SANA_SLURM_PARTITION` must be set in the environment."
        )
