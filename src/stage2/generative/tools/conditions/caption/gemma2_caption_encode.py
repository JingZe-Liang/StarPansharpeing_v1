...
from transformers.models import AutoTokenizer, Gemma2Model
...
def gemma2_caption_encode(
    ...
    text_encoder = Gemma2Model.from_pretrained(ckpt_path)
    text_encoder.to(device)

    select_index = [0] + list(range(-max_size + 1, 0))

    def encode(prompt: str):
        prompt = _text_preprocessing(prompt, clean_caption=False)
