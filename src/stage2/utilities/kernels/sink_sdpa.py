...
class AttentionSinkAtHome(torch.autograd.Function):
    ...
    def flash_attention_forward(
        ...
        (out, lse, cum_q, cum_k, max_q, max_k, seed, offset, _) = (
    ...
    def cudnn_attention_forward(
        ...
        ctx.save_for_backward(
            q, k, v, out, lse, cum_q, cum_k, philox_seed, philox_offset
        )
        ctx.max_q = int(max_q)
        ctx.max_k = int(max_k)
        return out, lse
    ...
    def backward(ctx, dOut, dLSE):
        if dOut is None:
            raise RuntimeError(
                "This wrapper requires grad wrt 'out' (dOut) for LSE grads injection."
            )
        if dLSE is None:
        ...
        match ctx.backend:
            case "flash":
                dQ, dK, dV = AttentionSinkAtHome.flash_attention_backward(
                    ctx, dOut, dLSE
                )
            case "cudnn":
                dQ, dK, dV = AttentionSinkAtHome.cudnn_attention_backward(
                    ctx, dOut, dLSE
                )
...
def attention_sink_at_home(
    ...
    *,
    ...
    o, lse = AttentionSinkAtHome.apply(
        q, k, v, float(dropout_p), bool(is_causal), scale, eps, backend
    )
    if s is not None:
        o = (
            torch.sigmoid(lse[..., None] - s[None, :, None, None]) * o.to(torch.float32)
        ).to(
            o.dtype
...
torch.testing.assert_close(
    vanilla_sdpa_torch, vanilla_sdpa_at_home, atol=atol, rtol=rtol
)
