...
from ..modules import Decoder3DType, DiscreteQuantizer, Encoder3DType
...
class CausalDiscreteVideoTokenizer(nn.Module):
    def __init__(
        ...
        if quantizer_name == DiscreteQuantizer.VQ.name:
            assert "num_embeddings" in kwargs, (
            ...
            assert "codebook_size" in kwargs, (
                f"`codebook_size` must be provided for {quantizer_name}."
            )
            assert "codebook_dim" in kwargs, (
            ...
            assert "num_quantizers" in kwargs, (
