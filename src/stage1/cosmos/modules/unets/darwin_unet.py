...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        self.emb_layers = nn.Sequential(
            SiLU(),
            nn.Linear(emb_channels, self.out_channel),
        )
        self.z_emb_layers = nn.Sequential(
            SiLU(),
            nn.Conv2d(emb_channels, self.out_channel, 1),
        )
        self.cond_fuse_layer = nn.Conv2d(
            self.out_channel,
            self.out_channel * 2 if use_scale_shift_norm else self.out_channel,
            1,
        )
        ...
        self.residual_factor = nn.Parameter(torch.zeros(1, self.out_channel, 1, 1))
    ...
    def forward(self, x, emb, z):
        ...
        # old fashion activation checkpoint
        # return checkpoint(
        #     self._forward, (x, emb, z), self.parameters(), self.use_checkpoint
        # )
        if self.use_checkpoint and self.training:
            return checkpoint_torch(self._forward, x, emb, z, use_reentrant=True)
        else:
            return self._forward(x, emb, z)
    ...
    def _forward(self, x, emb, z):
        ...
        # shape match
        z = F.interpolate(z, size=h.shape[-2:], mode="nearest")

        t_out = self.emb_layers(emb).type(h.dtype)
        z_out = self.z_emb_layers(z)
        # fuse
        emb_out = self.cond_fuse_layer(z_out + t_out[..., None, None])  # 2d
        ...
        if self.use_scale_shift_norm:
            ...
            h = h + emb_out
            h = self.out_layers(h)  # zero init, or use learnable ratio control

        return self.skip_connection(x) + h * self.residual_factor
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.residual_factor = nn.Parameter(torch.zeros(1, channels, 1))

    def forward(self, x):
        # return checkpoint(self._forward, (x,), self.parameters(), True)
        if self.use_checkpoint:
            return checkpoint_torch(self._forward, x, use_reentrant=True)
        else:
    ...
    def _forward(self, x):
        ...
        h = self.proj_out(h) * self.residual_factor
...
class QKVAttention(nn.Module):
    ...
    def __init__(self, n_heads, backend: str = "sdpa"):
        super().__init__()
        self.n_heads = n_heads
        self.backend = backend

    def forward_math(self, qkv):
        """
        Apply QKV attention.
    ...
    def forward(self, qkv):
        if self.backend == "math":
            return self.forward_math(qkv)
        elif self.backend == "sdpa":
            return self.forward_sdpa(qkv)
        else:
            raise ValueError("backend must be math or sdpa")
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        **_discard_kwargs,
        ...
        self.dropout = dropout
        self.t_embed_type = t_embed_type
        self.channel_mults = channel_mults
        self.conv_resample = conv_resample
        self.use_checkpoint = act_checkpoint
        ...
        # z embedder
        self.z_embedding = nn.Conv2d(z_channels, cond_embed_dim, 3, 1, 1)
        ...
        for level, mult in enumerate(channel_mults):
            for _ in range(res_blocks):
                ...
                if ds in attn_res:
                    logger.info(f"[Encoder]: {ds=} when {attn_res=}")
                    layers.append(
                        AttentionBlock(
                            ch,
                            use_checkpoint=act_checkpoint,
        ...
        for level, mult in list(enumerate(channel_mults))[::-1]:
            for i in range(res_blocks + 1):
                ...
                if ds in attn_res:
                    logger.info(f"[Decoder]: {ds=} when {attn_res=}")
                    layers.append(
                        AttentionBlock(
                            ch,
                            use_checkpoint=act_checkpoint,
        ...
        z_res = image_size // _discard_kwargs.get("spatial_compression", 8)
        self.null_cond = nn.Parameter(torch.zeros(1, z_channels, z_res, z_res))
        torch.nn.init.trunc_normal_(self.null_cond, std=0.02)

    def embed_cfg_z(self, z):
        bs = z.shape[0]
    ...
    def forward(
        ...
        if self.t_embed_type == "gamma":
            emb = self.t_embedder(gamma_embedding(gammas, self.inner_channel))
        else:
            emb = self.t_embedder(gammas)
        z = self.embed_cfg_z(z)

        # h = x.type(torch.float32)  # not used
        h = x
...
if __name__ == "__main__":
    from src.utilities.network_utils import func_mem_wrapper
    ...
    x = torch.randn((b, c, h, w)).to(device, dtype)
    z = torch.randn((b, 16, h // 8, w // 8)).to(device, dtype)
    emb = torch.ones((b,)).float().to(device, dtype)
    ...
    def test_fb():
        ...
        with torch.autocast(device, dtype=dtype):
            # count any
            # analysis = FlopCountAnalysis(model, (x, emb, z))
