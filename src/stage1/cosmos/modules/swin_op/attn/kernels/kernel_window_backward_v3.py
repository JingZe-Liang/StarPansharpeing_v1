...
@triton.autotune(configs=_BWD_AUTOTUNE_CONFIGS, key=["seq_pad", "head_dim"])
@triton.jit
def _window_bwd_kernel_v3(
    Q,
    K,
    V,
    bias,
    ...
    batch_local_id = tl.program_id(0)
    head_id = tl.program_id(1)
    batch_global_id = batch_start + batch_local_id

    stride_head = seq * head_dim
    stride_batch = stride_head * head
    offset = batch_global_id * stride_batch + head_id * stride_head
    ...
    if window_mask is not None:
        mask_ptr = tl.make_block_ptr(
            base=window_mask + batch_global_id * seq * seq,
            shape=(seq, seq),
            strides=(seq, 1),
            offsets=(0, 0),
            block_shape=(seq_pad, seq_pad),
    ...
    if bias is not None:
        bias_partial_index = (
            (batch_local_id * head + head_id) * seq * seq
            + tl.arange(0, seq_pad)[:, None] * seq
            + tl.arange(0, seq_pad)[None, :]
        )
        d_bias_partial_ptr = d_bias_partial + bias_partial_index
        tl.store(d_bias_partial_ptr, d_logits, mask=valid_2d)
