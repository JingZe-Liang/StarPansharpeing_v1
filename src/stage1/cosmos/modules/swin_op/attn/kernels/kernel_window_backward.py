...
def _window_bwd_kernel(
    ...
    # load shifted-window mask for current batch window
    ...
    for _ in range(head_chunk):
        ...
        d_q_data = (tl.dot(d_attn, k_data) * scale_qk).cast(q_data.dtype)
        tl.store(d_Q_ptr, d_q_data, mask=mask[:, None])

        d_k_data = (tl.dot(d_attn.trans(1, 0), q_data) * scale_qk).cast(k_data.dtype)
