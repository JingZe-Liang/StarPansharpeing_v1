...
from ..rope import RotaryPositionEmbeddingPytorchV2
...
class Attention(nn.Module):
    ...
    def enable_kv_cache(self, bsz, max_seq_len) -> None:
...
def precompute_freqs_cis_2d(
    ...
    # (l, 2) -> (l * 2) @ (d//2) -> (l * 2, d//2) -> (l, d)
    freqs = torch.outer(t.flatten(), freqs).view(*t.shape[:-1], -1)  # (l, -1)
    # (l, d, 2)
...
def apply_rotary_emb(x: torch.Tensor, freqs_cis: torch.Tensor):
    ...
    # Split the needed seq len
    l = x.size(1)
    if freqs_cis.size(0) > l:
