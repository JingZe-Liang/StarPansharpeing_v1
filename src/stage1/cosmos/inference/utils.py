...
def load_jit_model_shape_matched(
    ...
    if part == "encoder":
        ...
        raise ValueError(f"part should be either 'encoder' or 'decoder', got {part}")

    try:
        missing_keys, unexp_keys = model.load_state_dict(state_dict, strict=False)
        logger.warning(f"missing keys: {missing_keys}, unexpected keys: {unexp_keys}")
        not_pretrained_keys = [*missing_keys, *unexp_keys]
        learnable_keys = []
        for key in model.state_dict().keys():
            if key not in not_pretrained_keys:
                learnable_keys.append(key)

        return model.to(device), {
            "not_pretrained_keys": not_pretrained_keys,
            "pretrained_keys": learnable_keys,
        }
        ...
        # mismatch or not in ckpt keys
        mismatch_keys = []
        not_in_ckpt_keys = []

        # loop through model state dict
        for name in model_state_dict.keys():
            if name not in ckpt_keys:
                ...
                raise RuntimeError(
                    f"unrecognized pattern mismatch: {name} - model of {model_state_dict[name].shape} != ckpt of {state_dict[name].shape}"
                )

        # info out
        logger.info(f"total mismatch: {len(mismatch_keys) + len(not_in_ckpt_keys)}")
        logger.info(f"shape mismatch keys: {mismatch_keys}")
        ...
        for key in model_state_dict.keys():
            if key not in not_pretrained_keys:
                learnable_keys.append(key)
        logger.info(f"missing keys: {missing_keys}, unexpected keys: {unexp_keys}")
