...
def main(args):
    ...
    ckpt_string_name = (
        os.path.basename(args.gpt_ckpt).replace(".pth", "").replace(".pt", "")
    )
    prompt_name = args.prompt_csv.split("/")[-1].split(".")[0].lower()
    folder_name = (
        f"{model_string_name}-{ckpt_string_name}-{prompt_name}-size-{args.image_size}-size-{args.image_size}-{args.vq_model}-"
    ...
    total_samples = int(
        math.ceil(num_fid_samples / global_batch_size) * global_batch_size
    )
    if rank == 0:
    ...
    for _ in pbar:
        ...
        for i in range(n):
            ...
            prompt_batch.append(
                prompt_list[index] if index < len(prompt_list) else "a cute dog"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt-csv", type=str, default="evaluations/t2i/PartiPrompts.tsv"
    )
    ...
    parser.add_argument(
        "--gpt-model", type=str, choices=list(GPT_models.keys()), default="GPT-XL"
    )
    ...
    parser.add_argument(
        ...
        choices=["c2i", "t2i"],
        default="t2i",
        help="class->image or text->image",
    )
    parser.add_argument(
        "--cls-token-num",
        type=int,
    ...
    parser.add_argument(
        "--sample-dir",
        type=str,
        default="samples_parti",
        help="samples_coco or samples_parti",
    )
    parser.add_argument("--per-proc-batch-size", type=int, default=32)
    parser.add_argument("--num-fid-samples", type=int, default=30000)
    parser.add_argument("--global-seed", type=int, default=0)
    parser.add_argument(
        "--top-k", type=int, default=1000, help="top-k value to sample with"
    )
    parser.add_argument(
        "--temperature",
        type=float,
