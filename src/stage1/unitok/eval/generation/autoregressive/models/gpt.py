...
class Attention(nn.Module):
    ...
    def forward(
        ...
        output = F.scaled_dot_product_attention(
            ...
            attn_mask=mask,
            is_causal=True
            if (mask is None and not full_attn)
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        h = x + self.drop_path(
            self.attention(
                self.attention_norm(x), freqs_cis, start_pos, mask, full_attn
            )
        )
...
class AutoRegressiveHead(nn.Module):
    def __init__(self, config):
        ...
        for _ in range(config.n_output_layer):
            self.layers.append(TransformerBlock(config, drop_path=0.0))
...
class Transformer(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.tok_embeddings = TokenEmbedder(
            config.vocab_size, config.dim, config.num_codebooks
        )
    ...
    def setup_head_caches(self, max_batch_size, max_seq_length, dtype, device):
        ...
        self.output.head_causal_mask = causal_mask.unsqueeze(0).repeat(
