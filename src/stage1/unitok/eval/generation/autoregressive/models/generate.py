...
def prefill(
    ...
    **sampling_kwargs,
    ...
    for i in range(model.num_codebooks):
        ...
        if i == 0:
            logits = model.output.forward_test(
                next_token, input_pos=start_pos, mask=mask
            )
        else:
            pred_idx = torch.cat([pred_idx, pred_idx])
            logits = model.output.forward_test(
                next_token, idx=pred_idx, input_pos=start_pos, mask=mask
            )
        if cfg_scale > 1.0:
...
def generate(
    ...
    **sampling_kwargs,
    ...
    seq = torch.empty(
        (max_batch_size, num_codebooks, T_new), dtype=torch.int, device=device
    )
