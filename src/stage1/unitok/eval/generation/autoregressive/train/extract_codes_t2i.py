...
def main(args):
    ...
    dataset = CustomDataset(
        args.data_path, args.data_start, args.data_end, transform=transform
    )
    ...
    for img, code_dir, code_name in loader:
        ...
        np.save(
            os.path.join(
                args.code_path, code_dir[0], "{}.npy".format(code_name.item())
            ),
            x,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image-size", type=int, choices=[256, 384, 448, 512], default=512
    )
