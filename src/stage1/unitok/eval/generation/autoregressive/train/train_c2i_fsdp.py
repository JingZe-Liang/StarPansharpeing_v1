...
def main(args):
    ...
    print(
        f"Starting rank={global_rank}, device={device}, seed={seed}, world_size={dist.get_world_size()}."
    )
    ...
    timestamp_tensor = torch.tensor(
        [timestamp] if timestamp is not None else [0.0], dtype=torch.double
    ).to(device)
    ...
    model_string_name = args.gpt_model.replace(
        "/", "-"
    )  # e.g., GPT/XL --> GPT-XL (for naming folders)
    ...
    # ======================================================
    optimizer = creat_optimizer_by_name(
        model, args.weight_decay, args.lr, (args.beta1, args.beta2), global_rank, logger
    )
    if args.gpt_resume:
        opt_state_world_size = len(
            [
                x
                for x in os.listdir(args.gpt_resume)
    ...
    logger.info(
        f"Dataset contains {len(dataset):,} images ({args.code_path}) "
    ...
    if args.schedule == "cosine":
        total_train_steps = math.ceil(
            args.epochs * len(dataset) / args.global_batch_size
        )
        ...
        for x, y in loader:
            ...
            if train_steps % args.ckpt_every == 0 and train_steps > 0:
                ...
                torch.save(
                    ...
                    os.path.join(cloud_checkpoint_path, opt_state_fn),
                )
                dist.barrier()
                ...
                if global_rank == 0:
                    with open(
                        os.path.join(cloud_checkpoint_path, "resume_step.txt"), "w"
                    ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=16384,
        help="vocabulary size of visual tokenizer",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="dropout_p of resid_dropout_p and ffn_dropout_p",
    )
    parser.add_argument(
        "--token-dropout-p",
        type=float,
        default=0.1,
        help="dropout_p of token_dropout_p",
    )
    parser.add_argument(
        "--drop-path-rate", type=float, default=0.0, help="using stochastic depth decay"
    )
    ...
    parser.add_argument(
        "--data-parallel", type=str, choices=["sdp", "fsdp", "hsdp"], default="fsdp"
    )
