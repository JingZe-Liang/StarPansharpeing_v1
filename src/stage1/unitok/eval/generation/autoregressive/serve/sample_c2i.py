...
def main(args):
    ...
    if args.cfg_scale > 1.0:
        prompt_token_ids.extend(
            [[args.num_classes] for _ in range(len(prompt_token_ids))]
        )
    # Create an LLM.
    llm = LLM(
        args=args,
    ...
    # decode to image
    index_sample = torch.tensor(
        [output.outputs[0].token_ids for output in outputs], device=device
    )
    if args.cfg_scale > 1.0:
    ...
    save_image(
        samples,
        "sample_{}_vllm.png".format(args.gpt_type),
        nrow=4,
        normalize=True,
        value_range=(-1, 1),
    )
