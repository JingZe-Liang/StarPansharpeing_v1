...
class AttentionMonkeyPatch(Attention):
    ...
    def __init__(self, config: ModelArgs):
        ...
        self.attn = pagedAttention(
            self.n_head, 112, 100**-0.5, num_kv_heads=self.n_kv_head
        )
    ...
    def forward(
        ...
        zero_pad = torch.zeros(
            xq.shape[0], self.n_head, 112 - 100, device=xq.device, dtype=xq.dtype
        )
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        h = x + self.attention(
            self.attention_norm(x), positions, kv_cache, attn_metadata
        )
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        token_embeddings = self.cls_embedding(cond_ids) * (
            positions.max() == 0
