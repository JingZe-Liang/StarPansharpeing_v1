...
def main(args):
    ...
    folder_name = (
        f"{args.vq_model}-{args.dataset}-size-{args.image_size}-size-{args.image_size_eval}"
    ...
    for x, _ in loader:
        if args.image_size_eval != args.image_size:
            rgb_gts = F.interpolate(
                x, size=(args.image_size_eval, args.image_size_eval), mode="bicubic"
            )
        else:
            rgb_gts = x
        rgb_gts = (
            rgb_gts.permute(0, 2, 3, 1).to("cpu").numpy() + 1.0
        ) / 2.0  # rgb_gt value is between [0, 1]
        ...
        for i, (sample, rgb_gt) in enumerate(zip(samples, rgb_gts)):
            ...
            )  # rgb_restored value is between [0, 1]
            psnr = psnr_loss(rgb_restored, rgb_gt)
            ssim = ssim_loss(
                rgb_restored, rgb_gt, multichannel=True, data_range=2.0, channel_axis=-1
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, choices=["imagenet", "coco"], default="imagenet"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--image-size-eval", type=int, choices=[256, 384, 512], default=256
    )
