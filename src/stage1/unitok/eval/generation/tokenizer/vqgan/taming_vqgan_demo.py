...
def main(args):
    ...
    out_path = (
        args.image_path.replace(".jpg", "_vqgan.jpg")
        .replace(".jpeg", "_vqgan.jpeg")
        .replace(".png", "_vqgan.png")
    )
    ...
    with torch.no_grad():
        ...
        output = model.decode_code(
            indices, latent.shape
        )  # output value is between [-1, 1]

    # postprocess
    output = F.interpolate(
        output, size=[size_org[1], size_org[0]], mode="bilinear"
    ).permute(0, 2, 3, 1)[0]
    sample = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="vqgan_openimage_f8_16384",
    )
