        ...
        if self.data_name == "coco2014":
            if self.load_txt_from_file:
                self.imagelist = self.natural_sort(
                    glob.glob(
                        os.path.join(self.data_dir, self.data_type, "*.%s" % "png")
                    )
                )
                ...
                self.data = CocoCaptions(
                    root=os.path.join(self.data_dir, "val2014"),
                    annFile=os.path.join(
                        self.data_dir, "annotations", "captions_val2014.json"
                    ),
                )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="coco2014",
        type=str,
        help="in [imagenet2012, coco2014, laion4k]",
    )
    parser.add_argument(
        "--ref_type", default="train/valid/test", help="Type of reference dataset"
    )
