...
def load_model(args):
    ...
    hf_hub_download(
        repo_id="FoundationVision/LlamaGen", filename=vq_ckpt, local_dir=ckpt_folder
    )
    hf_hub_download(
        repo_id="FoundationVision/LlamaGen", filename=gpt_ckpt, local_dir=ckpt_folder
    )
...
parser.add_argument(
    "--vq-model", type=str, choices=list(VQ_models_HF.keys()), default="VQ-16"
)
parser.add_argument(
    "--codebook-size",
    type=int,
