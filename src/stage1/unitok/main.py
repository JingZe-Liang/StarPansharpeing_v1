    ...
    if resume is not None:
        try:
            ...
            if resume_epoch == args.epoch:
                ...
                print(
                    f"[auto_resume success] resume from ep{resume_epoch}, it{resume_iter}"
                )
                return ckpt
...
def train_one_ep(
    ...
    metric_logger = MetricLogger(
        cur_epoch=epoch, total_epoch=args.epoch, delimiter="  "
    )
    [
        metric_logger.add_meter(x, SmoothedValue(window_size=1, fmt="{value:.2g}"))
        for x in ("glr", "dlr")
    ]
    [
        metric_logger.add_meter(x, SmoothedValue(window_size=1, fmt="{median:.2f}"))
        for x in ("gnm", "dnm")
    ]
    [
        metric_logger.add_meter(x, SmoothedValue(fmt="{median:.3f}"))
        for x in ("L1", "Lnll", "Ld", "Lc", "Wg")
    ]
    ...
    for cur_iter, sample in metric_logger.monitor_enumerate(
        dataloader, start_iter, num_iters, print_freq=100
    ):
        ...
        disc_lr_stats = (
            disc_scheduler.step(disc_global_iter) if disc_global_iter >= 0 else [0]
        )
        ...
        warmup_disc_schedule = (
            0 if disc_global_iter < 0 else min(1.0, disc_global_iter / disc_warmup_iter)
        )
        fade_blur_schedule = (
            0
            if disc_global_iter < 0
            else min(1.0, disc_global_iter / (disc_warmup_iter * 2))
        )
        ...
        if (cur_iter + 1) % eval_interval == 0:
            ...
            if dist.is_master() and any(
                v in data for v in ("imagenet-val", "imagenet-v2")
            ):
            ...
            if dist.is_master():
                ckpt_path = os.path.join(
                    args.output_dir, f"ckpt-ep{epoch}-iter{cur_iter}.pth"
                )
                torch.save(
                    {
...
def main():
    ...
    data = load_data(
        args, preprocess_fns, epoch=start_epoch, iters=start_iter, tokenizer=tokenizer
    )
    ...
    print(
        f"[model] UniTok #paras {sum(p.numel() for p in unitok.parameters()) / 1e6:.2f}"
    )
    ...
    # distributed wrapper
    unitok = DDP(
        unitok, device_ids=[dist.get_local_rank()], static_graph=args.ddp_static
    )
    ...
    print(
        f"[train] Total Training Time: {total_time},\t Lg: {stats['Lnll']:.3f},\t Ld: {stats['Ld']:.3f}"
    )
    ...
    if isinstance(sys.stdout, dist.BackupStreamToFile) and isinstance(
        sys.stderr, dist.BackupStreamToFile
    ):
