...
def load_checkpoint_text_only(model, checkpoint_path, strict=False):
    ...
    key_text_projection = (
        ...
        else "text_projection"
    )
    if (
        key_text_projection in model.state_dict().keys()
        and key_text_projection in filterd_state_dict.keys()
    ):
        if (
            model.state_dict()[key_text_projection].shape
            != filterd_state_dict[key_text_projection].shape
        ):
