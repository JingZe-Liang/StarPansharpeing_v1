...
class LPIPS(nn.Module):
    ...
    def __init__(
        self, lpips_path, use_dropout=False
    ):  # do not use dropout by default because we use .eval mode by default
        super().__init__()
        # build models
        self.net = Vgg16(requires_grad=False)
        self.lins = nn.ModuleList(
            [NetLinLayer(c, use_dropout=use_dropout) for c in [64, 128, 256, 512, 512]]
        )  # c: vgg16 feature dimensions
    ...
    def forward(self, inp, rec):
        ...
        B = inp.shape[0]
        inp_and_recs = (
            torch.cat((inp, rec), dim=0).sub(self.shift).mul_(self.scale_inv)
        )  # first use dataset_mean,std to denormalize to [-1, 1], then use vgg_inp_mean,std to normalize again
        inp_and_recs = self.net(
            inp_and_recs
        )  # inp_and_recs: List[Tensor], len(inp_and_recs) == 5
        diff = 0.0
        for inp_and_rec, lin in zip(inp_and_recs, self.lins):
            diff += lin.model(
                (
                    normalize_tensor(inp_and_rec[:B])
                    - normalize_tensor(inp_and_rec[B:])
                ).square_()
            ).mean()
        return diff
...
class Vgg16(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        self.slice1 = torch.nn.Sequential(
            *[vgg_pretrained_features[x] for x in range(4)]
        )
        self.slice2 = torch.nn.Sequential(
            *[vgg_pretrained_features[x] for x in range(4, 9)]
        )
        self.slice3 = torch.nn.Sequential(
            *[vgg_pretrained_features[x] for x in range(9, 16)]
        )
        self.slice4 = torch.nn.Sequential(
            *[vgg_pretrained_features[x] for x in range(16, 23)]
        )
        self.slice5 = torch.nn.Sequential(
            *[vgg_pretrained_features[x] for x in range(23, 30)]
        )
        self.N_slices = 5
...
def main():
    ...
    l = LPIPS(
        r"C:\Users\16333\Desktop\PyCharm\vgpt\_vqgan\lpips_with_vgg.pth",
    ...
    x, y = (
        torch.load(r"C:\Users\16333\Desktop\PyCharm\vgpt\_vqgan\x.pth"),
        torch.load(r"C:\Users\16333\Desktop\PyCharm\vgpt\_vqgan\y.pth"),
    )
