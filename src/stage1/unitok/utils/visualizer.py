...
class Visualizer(object):
    def __init__(self, enable: bool, device, trainer):
        ...
        if enable:
            ...
            self.cmap_seq = ListedColormap(
                sns.color_palette("ch:start=.2, rot=-.3", as_cmap=True).colors[::-1]
            )
    ...
    def vis(self, epoch, png_path="", report_wandb=False) -> Tuple[float, float]:
        ...
        with torch.inference_mode():
            if hasattr(self.trainer, "unitok"):
                ...
                rec_B3HW = misc.unwrap_model(
                    self.trainer.unitok
                ).img_to_reconstructed_img(self.inp_B3HW)
                self.trainer.unitok.train()
            else:
                self.trainer.vae.eval()
                rec_B3HW = misc.unwrap_model(self.trainer.vae).img_to_reconstructed_img(
                    self.inp_B3HW
                )
        ...
        cmp_grid = torchvision.utils.make_grid(
            self.denormalize(torch.cat((self.inp_B3HW, rec_B3HW), dim=0)),
            padding=0,
            pad_value=1,
            nrow=B,
        )
        ...
        print(
            f"[vis] {L1=:.3f}, {Lpip=:.3f}, cost={time.time() - vis_stt:.2f}s",
            force=True,
        )
...
def get_boundary(
    patch_size, needs_loss, boundary_wid=3
):  # vis_img: BCHW, needs_loss: BL
    """
    ...
    boolmap = boolmap.repeat_interleave(patch_size, dim=2).repeat_interleave(
        patch_size, dim=3
    )  # B1hw => B1HW
