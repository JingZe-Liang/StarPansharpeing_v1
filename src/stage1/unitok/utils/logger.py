...
class SmoothedValue(object):
    ...
    def time_preds(self, counts) -> Tuple[float, str, str]:
        ...
        time_str2 = time.strftime(
            "%Y-%m-%d %H:%M", time.localtime(time.time() + remain_secs)
        )
        ...
        if isinstance(dataloader, Iterator):
            for cur_iter in range(start_iter, num_iters):
                ...
                if cur_iter % print_freq == 0:
                    eta_iters = (
                        self.total_epoch - self.cur_epoch
                    ) * num_iters - cur_iter
...
class ProfileLogger(object):
    ...
    def log(self, cur_iter):
        if (cur_iter + 1) % self.log_freq == 0:
            self.speed_ls.append(
                (time.perf_counter() - self.last_t_perf) / self.log_freq
            )
            iter_speed = float(np.median(self.speed_ls))
            img_per_sec = self.bs / iter_speed
            img_per_day = img_per_sec * 3600 * 24 / 1e6
            self.max_nvidia_smi = max(
                self.max_nvidia_smi,
                max(gpu.memoryUsed for gpu in GPUtil.getGPUs()) / 1024,
            )
            mem_infos_dict = torch.cuda.memory_stats()
            memory_allocated = round(
                mem_infos_dict["allocated_bytes.all.current"] / 1024**3, 2
            )
            memory_reserved = round(
                mem_infos_dict["reserved_bytes.all.current"] / 1024**3, 2
            )
            max_memory_allocated = round(
                mem_infos_dict["allocated_bytes.all.peak"] / 1024**3, 2
            )
            max_memory_reserved = round(
                mem_infos_dict["reserved_bytes.all.peak"] / 1024**3, 2
            )
            num_alloc_retries = mem_infos_dict["num_alloc_retries"]

            tails = list(self.speed_ls)[-10:]
            print(
                colorama.Fore.LIGHTCYAN_EX + f"[profiling]  "
...
def get_param_for_log(
    model_name_3letters: str, named_parameters
) -> Dict[str, List[float]]:
    ...
    for n, p in named_parameters:
        ...
        if "gpt" in model_name_3letters:
            ...
            "disc" in model_name_3letters or "dsc" in model_name_3letters
        ):  # discriminator
            if "dwt" in n:
                tag = "0-dwt"
            elif "from" in n:
