...
class Args(Tap):
    ...
    hflip: bool = False
    workers: int = (
        8  # num workers; 0: auto, -1: don't use multiprocessing in DataLoader
    )
    ...
    vae_init: float = (
        -0.5
    )  # <0: xavier_normal_(gain=abs(init)); >0: trunc_normal_(std=init)
    ...
    local_bs: int = (
        64  # batch size per device; if this is specified, --global_bs will be ignored
    )
    ...
    lr_start_ratio: float = 0.0  # lr warmup: initial lr ratio
    ...
    e_temp: float = 0.01
    gada: int = 1
    bcr: float = 4.0  # balanced Consistency Regularization, used on small dataset with low reso, StyleSwin: 10.0
...
def init_dist_and_get_args():
    ...
    if args.local_bs == 0:
        args.local_bs = max(
            1, round(args.global_bs / args.grad_accu / dist.get_world_size())
        )
