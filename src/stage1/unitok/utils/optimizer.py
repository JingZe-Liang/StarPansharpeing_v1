...
class LAMBtimm(Optimizer):
    ...
    def step(self, closure=None):
        ...
        global_grad_norm = torch.full(
            size=(1,), fill_value=1e-12, dtype=torch.float32, device=device
        )
        ...
        max_grad_norm = torch.tensor(
            self.defaults["max_grad_norm"], dtype=torch.float32, device=device
        )
    ...
    def __init__(
        ...
        if self.fp16 or self.bf16:
            ...
            self.amp_ctx = torch.autocast(
                "cuda", enabled=True, dtype=dtype, cache_enabled=self.fsdp == 0
            )
            self.scaler = (
    ...
    def backward_clip_step(
        ...
        if stepping:
            ...
            if self.scaler is not None:
                ...
                if (
                    scaler_sc > 65536.0
                ):  # fp16 will overflow when >65536, so multiply 65536 could be dangerous
                    self.scaler.update(new_scale=65536.0)
                else:
                    self.scaler.update()
                try:
    ...
    def log_param(self, ep: int, tb_lg: misc.TensorboardLogger):
        if not self.fsdp:
            for name, values in get_param_for_log(
                self.model_name, self.model.named_parameters()
            ).items():
                if len(values) == 1:  # e.g., cls token will only have one value
                    values.append(values[0])
                tb_lg.log_tensor_as_distri(
                    name, torch.tensor(values, dtype=torch.float32), step=ep + 1
                )
...
def partition_param_groups(
    model, lr, wd, no_wd_keys=(), custom_lr_keys=(), custom_lr_multiplier=1.0
):
    ...
    params_w_wd = [
        ...
        if not no_wd(n, p) and not custom_lr(n) and p.requires_grad
    ]
    params_wo_wd = [
        p
        for n, p in model.named_parameters()
        if no_wd(n, p) and not custom_lr(n) and p.requires_grad
    ]
    custom_params_w_wd = [
        p
        for n, p in model.named_parameters()
        if not no_wd(n, p) and custom_lr(n) and p.requires_grad
    ]
    custom_params_wo_wd = [
        p
        for n, p in model.named_parameters()
        if no_wd(n, p) and custom_lr(n) and p.requires_grad
    ]
    ...
    if len(params_wo_wd) > 0:
        para_groups.append({"params": params_wo_wd, "weight_decay": 0.0, "lr": lr})
    if len(custom_params_w_wd) > 0:
        para_groups.append(
            {
                "params": custom_params_w_wd,
                "weight_decay": wd,
                "lr": lr * custom_lr_multiplier,
            }
        )
    if len(custom_params_wo_wd) > 0:
        para_groups.append(
            {
                "params": custom_params_wo_wd,
                "weight_decay": 0.0,
                "lr": lr * custom_lr_multiplier,
            }
        )
...
def build_optimizer(args, model_name, model):
    if model_name == "dis":
        opt_beta, lr, wd, grad_clip = (
            args.disc_optim_beta,
            args.disc_lr,
            args.disc_wd,
            args.grad_clip,
        )
    ...
    para_groups = partition_param_groups(
        model, lr, wd, no_wd_keys, custom_lr_keys, args.custom_lr_multiplier
    )
    beta1, beta2 = map(float, opt_beta.split("_"))
    optim = {
        "adamw": partial(torch.optim.AdamW, betas=(beta1, beta2), fused=args.fuse_opt),
        "lamb": partial(
            LAMBtimm, betas=(beta1, beta2), max_grad_norm=grad_clip
        ),  # eps=1e-7
        "lion": partial(
            Lion, betas=(beta1, beta2), max_grad_norm=grad_clip
        ),  # eps=1e-7
    }[args.optimizer]
