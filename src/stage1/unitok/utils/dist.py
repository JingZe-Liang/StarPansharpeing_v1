...
def __initialize(
    fork=False, backend="nccl", gpu_id_if_not_distibuted=0, timeout_minutes=30
):
    global __device
    ...
    tdist.init_process_group(
        backend=backend, timeout=datetime.timedelta(seconds=timeout_minutes * 60)
    )
...
def init_distributed_mode(local_out_path, only_sync_master=False, timeout_minutes=30):
    ...
    if (
        (is_master() if only_sync_master else is_local_master())
        and local_out_path is not None
        and len(local_out_path)
    ):
        sys.stdout, sys.stderr = (
            BackupStreamToFile(local_out_path, for_stdout=True),
            BackupStreamToFile(local_out_path, for_stdout=False),
        )
...
class BackupStreamToFile(object):
    def __init__(self, local_output_dir, for_stdout=True):
        ...
        fname = os.path.join(
            local_output_dir,
            "backup1_stdout.txt" if for_stdout else "backup2_stderr.txt",
        )
