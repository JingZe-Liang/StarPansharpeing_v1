    ...
    def __init__(self, args):
        ...
        if args.quant_proj == "linear":
            ...
            self.quant_proj = AttnProjection(
                ...
                args.vocab_width,
                self.encoder.embed_dim // args.vocab_width,
            )
        else:
    ...
    def forward(self, img, vae_bs, text=None, ret_usages=False):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            img_tokens = torch.utils.checkpoint.checkpoint(
                self.quant_proj, img_tokens, use_reentrant=False
            )
            img_tokens, vq_loss, entropy_loss, usages = self.quantizer(img_tokens)
            img_tokens = torch.utils.checkpoint.checkpoint(
                self.post_quant_proj, img_tokens, use_reentrant=False
            )
...
if __name__ == "__main__":
    ...
    model.pos_embed = nn.Parameter(
        torch.zeros(1, 1, model.embed_dim), requires_grad=False
    )
