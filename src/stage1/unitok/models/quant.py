...
def get_entropy_loss(latent_embed, codebook_embed, inv_entropy_tau):
    E_dist = latent_embed.square().sum(
        dim=1, keepdim=True
    ) + codebook_embed.square().sum(dim=1, keepdim=False)
    E_dist.addmm_(
        latent_embed, codebook_embed.T, alpha=-2, beta=1
    )  # E_dist: (N, vocab_size)
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, features):
        ...
        # calc loss
        vq_loss = F.mse_loss(features_hat.detach(), features).mul_(
            self.beta
        ...
        handler = (
            tdist.all_reduce(prob_per_class_is_chosen, async_op=True)
        ...
        vocab_usage = (
            (prob_per_class_is_chosen > 0.01 / self.vocab_size).float().mean().mul_(100)
        )
...
class VectorQuantizerM(nn.Module):
    ...
    def forward(self, features):
        latent_features = []
        global_vq_loss = 0.0
        global_entropy_loss = 0.0
        global_vocab_usage = 0.0
        chunk_size = features.shape[-1] // self.num_codebooks
        splited_features = features.split(chunk_size, dim=-1)
        for i, codebook in enumerate(self.codebooks):
            latent_feature, vq_loss, entropy_loss, vocab_usage = codebook(
                splited_features[i]
            )
            latent_features.append(latent_feature)
