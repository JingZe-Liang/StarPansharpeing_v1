...
import torch
from fvcore.nn import parameter_count as count_parameters
...
from .train import linear_warmup_cosine_decay
...
class SSLMetaArch(nn.Module):
    ...
    def __init__(
        ...
        if cfg.distillation.enabled:
            # TODO: fix the teacher default cfg pass to the method.
            self._setup_distillation(teacher_backbone, embed_dim, default_cfg=None)
    ...
    def init_weights(self) -> None:
        ...
        if self.cfg.student.resume_from_teacher_chkpt:
            ...
            init_fsdp_model_from_checkpoint(
                ...
                prefixes_not_sharded=["backbone.rope_embed.periods"],
                process_group=get_process_subgroup(),
            )
            self.model_ema.load_state_dict(self.student.state_dict())
    ...
    def forward_backward(
        self, data: dict, *, teacher_temp: float, iteration=0, **ignored_kwargs
    ...
    def prepare_for_distributed_training(self) -> None:
        process_subgroup = get_process_subgroup()
        default_process_group = get_default_process_group()
    ...
    def broadcast_to_subgroups(self, tensor, over_dim, global_batch_size=None):
        ...
        return catted.chunk(subgroup_size, dim=over_dim)[get_subgroup_rank()].clone()
