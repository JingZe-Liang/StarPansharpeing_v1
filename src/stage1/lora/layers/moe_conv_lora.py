...
class LoRAMoELinearDPSK(nn.Linear, LoRALayer): ...
...
class ConvLoRALinear(nn.Linear, LoRALayer):
    ...
    Note: the experts is convolution and the MoE is on batch level.
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            if moe_type == "vanilla":
                ...
                self.lora_moe_gating = MoEGateDPSK(
                    num_experts_per_tok=conv_lora_expert_num,
                    n_routed_experts=topk,
                    hidden_size=self.r,
    ...
    def forward_dpsk_moe(self, x: torch.Tensor):
        result = F.linear(x, self.T(self.weight), bias=self.bias)
        _orig_shape = x.shape
        if self.r > 0:
            ...
            if dim == 3:
                B, L, C = lora_res.size()
                H = W = int(math.sqrt(L))
                assert H * W == L, "The input length must be a perfect square"
            ...
            # ep=1
            ep_hidden_states = x.repeat_interleave(  # [bs * topk, c, h, w]
                self.topk, dim=0
            )
            y = torch.empty(ep_hidden_states, dtype=x.dtype, device=x.device)
            for i, expert in enumerate(self.lora_moe_experts):
                assert expert is not None
                inp = ep_hidden_states[flat_topk_idx == i]
                if self.upsample_ratios[i] != 1:
                    inp = F.interpolate(
                        inp,
                        scale_factor=self.upsample_ratios[i],
                        mode="bicubic",
                        align_corners=False,
                    )
                exp_y = expert(inp).to(x.dtype)
                if self.upsample_ratios[i] != 1:
                    y = F.interpolate(
                        y,
                        size=(int(H), int(W)),
                ...
                y[flat_topk_idx == i] = exp_y

            # reshape
            _topk_weight_shape = topk_weight.shape
            y = y.view(*_topk_weight_shape, -1)  # [bs, topk, c, h, w]
            topk_weight: torch.Tensor
            y = y * topk_weight[..., None, None, None]  # [bs, topk, c, h, w]
            y = y.sum(dim=1)  # [bs, c, h, w]
            y = y.permute(0, 2, 3, 1)
            y = y.to(x).view(_orig_shape)

            # apply load balancing loss
            if self.training:
                y = AddAuxiliaryLoss.apply(y, moe_loss)

            result += (y @ self.lora_B.T) * self.scaling

        return result, moe_loss

    def forward_vanilla(self, x: torch.Tensor):
        result = F.linear(x, self.T(self.weight), bias=self.bias)
        if self.r > 0:
            lora_res = self.lora_dropout(x) @ self.lora_A.T
            dim = lora_res.dim()
            ...
            # Calculate the gating values.
            lora_res = lora_res.permute(0, 3, 1, 2).contiguous()  # [bs, c, h, w]
            gates, moe_loss = self.lora_moe_gating(lora_res)  # gates: [bs, m]

            # Distribute data samples to experts.
            dispatcher = SparseDispatcher(self.num_experts, gates)
            expert_inputs = dispatcher.dispatch(lora_res)  # [bs, c, h, w] * num_experts
            expert_outputs = []
...
class MoEGate(nn.Module):
    ...
    def forward(self, feats, loss_coef=1e-2, noise_epsilon=1e-2):
        ...
        feats_S = self.gap(feats).view(batch_size, -1)  # [bs, c]

        clean_logits = feats_S @ self.w_gate  # [bs, m]
        ...
        top_logits, top_indices = logits.topk(min(self.k + 1, self.M), dim=1)  # [bs, k]
        top_k_logits = top_logits[:, : self.k]
        top_k_indices = top_indices[:, : self.k]
        top_k_gates = self.softmax(top_k_logits)
        zeros = torch.zeros_like(logits, requires_grad=True).float()  # [bs, m]
...
if __name__ == "__main__":
    moe_linear_lora = ConvLoRALinear(
        512,
        512,
        r=8,
        lora_alpha=1,
    ...
    x = torch.randn(1, 256, 512)  # [bs, s, c]
    out, moe_loss = moe_linear_lora(x)
    print(out.shape)

    print("-" * 30)

    conv_lora = LoRAConv2d(
        512,
        512,
        kernel_size=3,
        r=8,
    ...
    x = torch.randn(1, 512, 256, 256)  # [bs, c, h, w]
    out = conv_lora(x)
    print(out.shape)
