...
class EmbedND(nn.Module):
    def __init__(self, dim: int, theta: int, axes_dim):
        ...
        self.dim = dim
        self.theta = theta
        self.axes_dim = axes_dim  # [8, 28, 28]

    def forward(self, ids: Tensor) -> Tensor:
        n_axes = ids.shape[-1]  # 3

        # rope(ids[..., 0], 8, 10_000)
        # rope(ids[..., 1], 28, 10_000)
        # rope(ids[..., 2], 28, 10_000)
        emb = torch.cat(
...
def prepare_idxs(img, code_length, patch_size):
    ...
    txt_ids = (
        torch.zeros((bs, code_length, 3), device=img.device)  # (b, t, 3)
        + torch.arange(code_length, device=img.device)[None, :, None]  # (1, t, 1)
    )
...
class FlowMo(nn.Module):
    def __init__(self, width, config):
        ...
        self.image_channels = getattr(config.data, "image_channels", 3)
        self.image_size = config.data.image_size
        self.patch_size = config.model.patch_size
        self.code_length = code_length
        self.dit_mode = getattr(config.model, "model_type", "dit_b_4")
    ...
    def forward(
        ...
        if self.config.model.enable_cfg and enable_cfg:
            ...
            cfg_mask = (torch.rand((b,), device=code.device) > 0.1)[
                :, None, None
            ]  # (b, 1, 1)
            code = code * cfg_mask
    ...
    def reconstruct_checkpoint(self, code):
        with torch.autocast(
            ...
            z = torch.randn(
                (bs, self.image_channels, self.image_size, self.image_size)
            ).cuda()
            ts = (
...
def rf_loss(config, model, batch, aux_state=None):
    x = batch["image"]
    b = x.size(0)
...
def rf_sample(
    ...
    images = []
    for i, (t, dt) in tqdm(enumerate((zip(ts, dts))), leave=False):
