...
class RAR(BaseModel):
    ...
    def forward_fn(
        self, input_ids, condition, return_labels=False, orders=None, is_sampling=False
    ):
        ...
        if self.blocks[0].attn.kv_cache:
            if (
                self.blocks[0].attn.k_cache is not None
                and self.blocks[0].attn.v_cache is not None
            ):
