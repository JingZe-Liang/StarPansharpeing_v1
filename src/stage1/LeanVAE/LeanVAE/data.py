...
class MultiSizeVideoDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        video_path = (
            ...
            else os.path.join(self.data_folder, self.annotations[idx])
        )
        try:
        ...
        if total_frames < self.sequence_length:
            raise RuntimeError(
                f"Video {video_path} has only {total_frames} frames, but {self.sequence_length} are required."
            )
...
def group_data_fun(lengths, generator=None):
    ...
    counter = Counter(
        lengths
    )  # counter {'1x256x256': 3, ''}   lengths ['1x256x256', '1x256x256', '1x256x256', ...]
    ...
    sorted_indices = [
        grouped_indices[item]
        for (item, _) in sorted(counter.items(), key=lambda x: x[1], reverse=True)
    ]
    ...
    for i_megabatch, megabatch in enumerate(shuffled_megabatches):
        ...
        for i_batch, batch in enumerate(megabatch):
            ...
            idx_length_dict = dict(
                [*zip(batch, len_each_batch)]
            )  # {0: '1x256x256', 100: '1x256x256'}
            count_dict = Counter(
                len_each_batch
            )  # {'1x256x256': 2} or {'1x256x256': 1, '1x768x256': 1}
            if len(count_dict) != 1:
...
def get_length_grouped_indices(
    ...
    split_to_even_chunks_len = [
        [[lengths[i] for i in batch] for batch in megabatch]
        for megabatch in megabatches
    ]
...
class LengthGroupedSampler(Sampler):
    ...
    def __init__(
        ...
        self.num_samples = (
            (len(lengths) + megabatch_size - 1) // megabatch_size
    ...
    def __iter__(self):
        ...
        megabatch_indices = get_length_grouped_indices(
            self.lengths,
            self.batch_size,
            self.world_size,
        ...
        indices = [
            i
            for batch in megabatch_indices[self.rank :: self.world_size]
            for i in batch
        ]
...
class VideoData(pl.LightningDataModule):
    ...
    def _dataset(self, train):
        datasets = []
        for dataset_path, train_list, val_list in zip(
            self.args.data_path, self.args.train_datalist, self.args.val_datalist
        ):
    ...
    def add_data_specific_args(parent_parser):
        ...
        parser.add_argument(
            ...
            type=str,
            nargs="+",
            default=["./video/kinetics-dataset/train/datapath"],
        )
        parser.add_argument(
            "--val_datalist",
            type=str,
            nargs="+",
            default=["./video/kinetics-dataset/val/datapath"],
        )
        ...
        parser.add_argument(
            "--dynamic_sample", action="store_true", help="Enable dynamic sampling rate"
        )
...
if __name__ == "__main__":
    ...
    lengths_dict = {
        file_name: lines(os.path.join(train_folder, file_name))
