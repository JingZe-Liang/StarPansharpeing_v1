...
from ..utils.patcher_utils import Patcher, Patcher2D, UnPatcher, UnPatcher2D
...
class LeanVAEConfig:
    ...
    latent_dim: int = 4
    ista_iter_num: int = 2
    ista_layer_num: int = 2
    l_dim: int = 128
    h_dim: int = 384
    sep_num_layer: int = 2
    fusion_num_layer: int = 4
    in_channels: int = 3
...
class LeanVAE(nn.Module):
    def __init__(self, args: LeanVAEConfig):
        super().__init__()
        self.args = args
        self.embedding_dim = args.embedding_dim

        self.latent_bottleneck = ISTA(
            points_num=args.embedding_dim,
            out_num=args.latent_dim,
            iter_num=args.ista_iter_num,
            layer_num=args.ista_layer_num,
        )
        ...
        self.encoder = Encoder_Arch(
            l_dim=args.l_dim,
            h_dim=args.h_dim,
            sep_num_layer=args.sep_num_layer,
            fusion_num_layer=args.fusion_num_layer,
        )
        ...
        self.chunksize_enc = (
            args.chunksize_enc
            if hasattr(args, "chunksize_enc") and args.chunksize_enc
            else 5
        )
        self.chunksize_dec = (
            args.chunksize_dec
            if hasattr(args, "chunksize_dec") and args.chunksize_dec
            else 5
        )
    ...
    def load_from_checkpoint(cls, ckpt_path, device="cpu", strict=False):
        ...
        if "args" not in checkpoint:
            raise ValueError(
                "Checkpoint does not contain 'args'. Ensure the checkpoint is saved correctly."
            )
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            "--latent_dim", type=int, default=4, help="Dimension of the latent channel."
        )
        parser.add_argument(
            "--ista_iter_num",
            type=int,
            default=2,
            help="Number of iterations in ISTA latent bottleneck.",
        )
        parser.add_argument(
            "--ista_layer_num",
            type=int,
            default=2,
            help="Number of layers in ISTA latent bottleneck.",
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=4,
            help="Number of fusion layers in encoder/decoder.",
        )
...
class LeanVAE2D(nn.Module):
    _no_split_modules = ["ResNAF", "NAF"]
    ...
    def __init__(self, args: LeanVAEConfig):
        ...
        self.dwt = Patcher2D()
        self.idwt = UnPatcher2D()

        self.encoder = Encoder_Arch2D(
            l_dim=args.l_dim,
            h_dim=args.h_dim,
            sep_num_layer=args.sep_num_layer,
            fusion_num_layer=args.fusion_num_layer,
    ...
    def decode(self, z, is_image=False):
        ...
        p_rec = self.latent_bottleneck.recon(z)
        x_dwt_rec = self.decoder.decode(p_rec)
        x_recon = self.idwt(x=x_dwt_rec)
        return x_recon
    ...
    def get_last_layer(self):
        return self.decoder.image_high_proj[0].weight
