...
class AutoEncoderEngine(pl.LightningModule):
    ...
    def forward(self, x, optimizer_idx=None, x_recon=None, log_image=False):
        ...
        if optimizer_idx == 1:
            ...
            recon_loss_low = (
                (
                    F.l1_loss(x_dwt_rec[0][:, :3], x_dwt[0][:, :3])
                    + F.l1_loss(x_dwt_rec[1][:, :3], x_dwt[1][:, :3])
                )
                * self.l1_weight
                * 0.05
            )
            ...
            valid_start_indices = torch.tensor(
                [x for x in range(T - k + 1) if x % 4 == 1]
            )
            start_idx = valid_start_indices[
                torch.randint(0, len(valid_start_indices), (B,))
            ]
            frame_idx = start_idx.unsqueeze(1) + torch.arange(k)
            frame_idx = torch.cat(
                (torch.zeros((B, 1), dtype=torch.int), frame_idx), dim=1
            ).to(self.device)

            frame_idx_selected = frame_idx.reshape(-1, 1, k + 1, 1, 1).repeat(
                1, C, 1, H, W
            )
            ...
            frames_recon = (
                frames_recon.permute(0, 2, 1, 3, 4).contiguous().view(-1, 3, H, W)
            )
            perceptual_loss = (
    ...
    def configure_optimizers(self):
        opt_ae = torch.optim.Adam(
            self.autoencoder.parameters(), lr=self.args.lr, betas=(0.5, 0.9)
        )
