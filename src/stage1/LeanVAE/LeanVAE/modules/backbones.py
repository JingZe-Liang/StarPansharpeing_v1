...
class PEG3D(nn.Module):
    def __init__(self, dim):
        ...
        self.ds_conv = nn.Conv3d(
            in_channels=dim, out_channels=dim, kernel_size=(3, 3, 3), groups=dim
        )
    ...
    def forward(self, x):
        ...
        if self.tile_inference:
            if self.is_first_chunk:
                x = F.pad(x, (1, 1, 1, 1, 2, 0), value=0.0)
            else:
                x = F.pad(x, (1, 1, 1, 1, 0, 0), value=0.0)
...
def ffd(dim, mult=4, dropout=0.0):
...
class NAF(nn.Module):
    def __init__(self, num_layer, dim, is_3d: bool = True):
        ...
        for _ in range(num_layer):
            ...
            peg_cls = PEG3D if is_3d else PEG2D
            self.dconv_layer.append(peg_cls(dim))
...
class Encoder_Arch(nn.Module):
    def __init__(
        ...
        self._build_linear_patch(
            in_channel=in_channel,
            out_channel_low=l_dim,
            out_channel_high=h_dim,
            pt=patch_size[0],
            ph=patch_size[1],
            pw=patch_size[2],
        )
    ...
    def _build_linear_patch(
        self, in_channel=3, out_channel_low=128, out_channel_high=384, pt=2, ph=4, pw=4
    ):
        patch_config = {
            "video_low": (pt, ph, pw),
            "video_high": (pt, ph, pw),
    ...
    def _linear_patch(self, x, proj_type):
        ...
        return getattr(self, f"{proj_type}_low_proj")(low_comp), getattr(
            self, f"{proj_type}_high_proj"
        )(high_comp)
...
class Encoder_Arch2D(nn.Module):
    def __init__(
        self,
        l_dim=128,
        h_dim=384,
        ...
        super(Encoder_Arch2D, self).__init__()
        ...
        self.low_layer = ResNAF(num_layer=sep_num_layer, dim=l_dim, is_3d=False)
        self.high_layer = ResNAF(num_layer=sep_num_layer, dim=h_dim, is_3d=False)
        self.fusion_layer = ResNAF(
            num_layer=fusion_num_layer, dim=l_dim + h_dim, is_3d=False
        )

    def _build_linear_patch(
        self, in_channel=3, out_channel_low=128, out_channel_high=384, ph=4, pw=4
    ):
        patch_config = {
            "image_low": (ph, pw),
            "image_high": (ph, pw),
        ...
        for name, (h, w) in patch_config.items():
            if "low" in name:
                in_dim = in_channel * h * w
                out_dim = out_channel_low
            else:
                out_dim = out_channel_high
                in_dim = in_channel * h * w * 3
            proj = nn.Sequential(
                Rearrange(
    ...
    def forward(self, x):
        x_low, x_high = self._linear_patch(x=x, proj_type="image")
        return self._feature_transform(low_x=x_low, high_x=x_high)
...
class Decoder_Arch(nn.Module):
    ...
    def _linear_unpatch(self, x, proj_type):
        low_comp, high_comp = (
            getattr(self, f"{proj_type}_low_proj")(x[0]),
            getattr(self, f"{proj_type}_high_proj")(x[1]),
        )
    ...
    def decode(self, x, is_image=False):
        ...
        if is_image:
            ...
            if self.tile_inference and not self.is_first_chunk:
                ...
                xi = self._linear_unpatch(
                    x=(low_x[:, :1], high_x[:, :1]), proj_type="image"
                )
                xv = self._linear_unpatch(
                    x=(low_x[:, 1:], high_x[:, 1:]), proj_type="video"
                )
                return (xi, xv)
...
class Decoder_Arch2D(nn.Module):
    def __init__(
        self,
        l_dim=128,
        h_dim=384,
        ...
        super(Decoder_Arch2D, self).__init__()
    ...
    def _build_linear_unpatch(
        ...
        for name, (h, w) in patch_config.items():
            if "low" in name:
                out_dim = in_channel * h * w
                in_dim = out_channel_low
            else:
                in_dim = out_channel_high
                out_dim = in_channel * h * w * 3
            proj = nn.Sequential(
                nn.Linear(in_dim, out_dim),
                Rearrange(
    ...
    def forward(self, x):
        low_x, high_x = self._feature_transform(x)
        x = self._linear_unpatch(x=(low_x, high_x), proj_type="image")
        return x
...
class ISTA(nn.Module):
    def __init__(
        ...
        phi_init = np.random.normal(
            0.0, (1 / points_num) ** 0.5, size=(out_num, points_num)
        )
        self.phi = nn.Parameter(torch.from_numpy(phi_init).float(), requires_grad=True)
        self.Q = nn.Parameter(
            torch.from_numpy(np.transpose(phi_init)).float(), requires_grad=True
        )
        ...
        for _ in range(self.iter_num):
            self.threshold.append(
                nn.Parameter(torch.Tensor([0.01]), requires_grad=True)
            )
    ...
    def recon(self, y):
        ...
        for i in range(self.iter_num):
            recon_r = recon - self.weights[i] * torch.mm(
                (torch.mm(recon, self.phi.t()) - y), self.phi
            )
            recon = recon_r.reshape(b, t, h, w, -1)
            recon = self.forward_l[i](recon)
            recon = torch.mul(
                torch.sign(recon), F.relu(torch.abs(recon) - self.threshold[i])
            )
...
class ISTA2D(nn.Module):
    def __init__(
        ...
        super(ISTA2D, self).__init__()
        phi_init = np.random.normal(
            0.0, (1 / points_num) ** 0.5, size=(out_num, points_num)
        )
        phi_init = torch.from_numpy(phi_init).float().contiguous()
        self.phi = nn.Parameter(phi_init, requires_grad=True)
        self.Q = nn.Parameter(phi_init.t().contiguous(), requires_grad=True)
        self.iter_num = iter_num
        self.forward_l = nn.ModuleList()
        self.backward_l = nn.ModuleList()

        for _ in range(self.iter_num):
            self.forward_l.append(NAF(num_layer=layer_num, dim=points_num, is_3d=False))
            self.backward_l.append(
                NAF(num_layer=layer_num, dim=points_num, is_3d=False)
            )
