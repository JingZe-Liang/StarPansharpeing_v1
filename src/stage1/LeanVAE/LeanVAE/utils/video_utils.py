...
def load_video_from_path_tvio(
    ...
    if fps != -1:
        ...
        resample_indices = np.linspace(0, vlen - 1, num_frames_to_sample).astype(int)
    ...
    if frm_sampling_strategy == "uniform":
        ...
        if num_frm % 2 == 0:
            frame_indices = np.array(
                range(center - num_frm // 2, center + num_frm // 2)
            ).astype(int)
        else:
            frame_indices = np.array(
                range(center - num_frm // 2, center + num_frm // 2 + 1)
            ).astype(int)
    ...
    raw_sample_frms = sampled_frms[frame_indices]
...
def load_video_from_path_decord(
    ...
    if fps != -1:
        ...
        num_frames_to_sample = int(duration * fps)
        resample_indices = np.linspace(0, len(vr) - 1, num_frames_to_sample).astype(int)
    ...
    if frm_sampling_strategy == "uniform":
        ...
        frame_indices = np.array(
            range(rand_start, rand_start + sep * num_frm, sep)
        ).astype(int)
