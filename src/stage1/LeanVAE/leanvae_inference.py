...
def main(args, model, video_path, save_path, video_name):
    ...
    regular_size = (
        2  # input range is [-0.5, 0.5] if regular_size = 2, [-1, 1] if regular_size = 1
    )
    with torch.no_grad():
        ...
        x_rec = (
            torch.clamp(x_rec, -(1.0 / regular_size), (1.0 / regular_size))
            + (1.0 / regular_size)
        ) * (127.5 * regular_size)
        x_rec = x_rec.to("cpu", dtype=torch.uint8)
        write_video(
            os.path.join(save_path, video_name), x_rec, fps=fps, options={"crf": "10"}
        )
