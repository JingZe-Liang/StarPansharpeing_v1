...
from dataset.video_dataset import ValidVideoDataset, DecordInit, Compose, ToTensorVideo
...
class EvalDataset(ValidVideoDataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "generated": self.transform(generated_video_tensor),
        }
...
def calculate_common_metric(args, dataloader, device):
    ...
    for batch_data in tqdm(
        ...
        if args.metric == "fvd":
            ...
            tmp_list = list(
                calculate_lpips(real_videos, generated_videos, args.device)[
                    "value"
                ].values()
            )
        score_list += tmp_list
...
def parse_args():
    ...
    parser.add_argument(
        "--real_video_dir", type=str, help=("the path of original videos`")
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fvd_method", type=str, default="styleganv", choices=["styleganv", "videogpt"]
    )
