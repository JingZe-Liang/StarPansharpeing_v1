...
class Conv2dSame(nn.Conv2d):
    ...
    def calc_same_pad(self, i: int, k: int, s: int, d: int) -> int:
        ...
        if i <= 0 or s <= 0:
            return 0
        res = max((math.ceil(i / s) - 1) * s + (k - 1) * d + 1 - i, 0)
        return res
...
class DiffBandsInputConvIn(nn.Module):
    def __init__(
        ...
        init_ks: int = 5,  #! Fix to match original NLayerDiscriminator (was 5)
        norm_type: str = "gn",
        downsample_at_first: bool = False,
    ):
        super().__init__()
        ...
        if basic_module == "conv":
            ...
            raise ValueError(
                f"[Disc] Unknown basic module type: {basic_module}. Supported types are: 'conv', 'conv_act_norm'."
            )
        ...
        for c in band_lst:
            self.in_modules["conv_in_{}".format(c)] = basic_module_fn(  # type: ignore
                ...
                kernel_size=init_ks,
                stride=2 if downsample_at_first else 1,
                padding=1 if downsample_at_first else init_ks // 2,
            )
...
class NLayerDiscriminatorv2(nn.Module):
    def __init__(
        ...
        log_print(
            ...
            f"num_stages={num_stages}, activation_fn={activation_fn}, "
            f"norm_type={norm_type}, blur_resample={blur_resample}, "
            f"blur_kernel_size={blur_kernel_size}"
        )

        in_channel_mult = (1,) + tuple(map(lambda t: 2**t, range(num_stages)))
        init_kernel_size = 5  #! Fix to match original NLayerDiscriminator (was 5)
        if activation_fn == "leaky_relu":
            activation = functools.partial(
                nn.LeakyReLU, negative_slope=0.2
            )  #! Fix negative_slope to match original (was 0.1)
        else:
            activation = nn.SiLU
        ...
        self.block_in = (
            ...
            if isinstance(num_channels, int)
                ...
                band_lst=num_channels,
                hidden_dim=hidden_channels,
                basic_module="conv_norm_act",
                init_ks=init_kernel_size,
                norm_type=norm_type,
                downsample_at_first=False,
            )
        )
        ...
        for i_level in range(num_stages):
            ...
            #     if not blur_resample
            #     else BlurBlock(
            #         out_channels, BLUR_KERNEL_MAP[blur_kernel_size]
            #     ),  #! Fix BlurBlock: add channels parameter
    ...
    #! Add weight initialization method to match original NLayerDiscriminator
...
if __name__ == "__main__":
    ...
    rank = accelerator.process_index  # 获取当前进程的rank
    ...
    patch_discriminator_v2 = NLayerDiscriminatorv2(
        num_channels=[3, 4],  # [3, 6, 8],
        hidden_channels=128,
        num_stages=4,
        activation_fn="leaky_relu",  #! Fix activation function name (was "leakyrelu")
    ).to(f"cuda:{rank}")  # 注意这里用to而不是cuda()

    # compute the toal number of parameters
    # total_params = sum(p.numel() for p in patch_discriminator_v2.parameters())
    # log_print(f"Total parameters in patch_discriminator_v2: {total_params:,}")
    from fvcore.nn import parameter_count_table

    print(parameter_count_table(patch_discriminator_v2))
