...
logger = logger.bind(_name_="Phi_s_distill")
...
def get_hadamard_matrix(feature_dim: int, allow_approx: bool = True):
    ...
    if not torch.allclose(f, torch.eye(feature_dim, dtype=H.dtype, device=H.device)):
        logger.error(f"Invalid Hadamard matrix! H @ H.T not equal to eye matrix.")
