    ...
    if normalize:
        ...
        if rescale == "clamp":
            ...
            img = per_channel_normalize(
                img, c_dim=(-2, -1) if img.dim() == 4 else (0, 1)
            )
        else:
        ...
        if rescale == "clamp":
            ...
            img = per_channel_add_min(
                img, hw_dim=(-2, -1) if img.dim() == 4 else (0, 1)
            )
        else:
            raise ValueError(f"Unsupported rescale method: {rescale} for {normalize=}")
...
def clip_img_to_webdataset(
    ...
    img_patches, img_coords = sliding_window(img_tensor, img_clip_size, img_stride)

    save_dtype = None
    for patch_idx, patch in enumerate(img_patches):
