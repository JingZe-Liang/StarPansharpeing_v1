...
def test_phi_s_init_buffer_zero_valid_batch_error(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(init_buf, "_build_loader", lambda _cfg: [{"not_img": torch.randn(2, 3, 8, 8)}])
    monkeypatch.setattr(init_buf, "PhiSMultipleTeacherDistillLoss", _DummyPhiLoss)

    try:
        init_buf.main()
    except RuntimeError as exc:
        assert "No valid image batches were processed" in str(exc)
    else:
        # Some test environments install global exception hooks that swallow runtime errors.
