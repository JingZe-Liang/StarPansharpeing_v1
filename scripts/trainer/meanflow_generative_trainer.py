...
from src.utilities.train_utils.state import LossMetricTracker, StepsCounter


class MeanFlowGenerativeTrainer:
    def __init__(self, cfg: DictConfig):
        ...
        self.setup_meanflow_model()  # setup the meanflow model

        # optimizers and lr schedulers
        self.flow_optim, self.flow_sched = self.get_optimizer_lr_scheduler()
        ...
        # mean flow loss
        self.mean_flow: MeanFlow = hydra.utils.instantiate(self.cfg.mean_flow)
    ...
    def prepare_for_training(self):
        ...
        if self._is_fsdp or self.accelerator.distributed_type in (
            ...
            self.flow_model = nn.SyncBatchNorm.convert_sync_batchnorm(self.flow_model)
        ...
        if self._is_fsdp and self.accelerator.is_fsdp2:
            ...
            self.flow_model.dtype = torch.float
    ...
    def ema_update(self, mode="flow"):
        assert mode == "flow"
        if self.no_ema:
            # not support ema when is deepspeed zero2 or zero3
            return

        self.ema_flow_model.update()

    @torch.no_grad()
    def forward_tokenizer(self, x: torch.Tensor, mode: str = "encode") -> dict:
    ...
    def train_flow_step(
        self,
        hrms_latent: torch.Tensor,
        condition: torch.Tensor | None = None,
    ):
        with self.accelerator.autocast():
            mean_flow_d = self.mean_flow.loss(self.flow_model, hrms_latent, condition)
            mf_loss, mse_error, model_x0 = (
                mean_flow_d["mf_loss"],
                mean_flow_d["mse"],
                mean_flow_d["model_x0"],
            )

        if self.accelerator.sync_gradients:
            # backward
            self.flow_optim.zero_grad()
            self.accelerator.backward(mf_loss)
            self.gradient_check(self.flow_model)
            self.flow_optim.step()

            # ema update
            self.ema_update(mode="flow")
    ...
    def train_step(self, batch: dict):
        ...
        with self.accelerator.accumulate(self.flow_model):
            ...
            if (
                ...
                self.log_msg(f"[Track Quality]: 'save model predicted x0")

                if self.accelerator.is_main_process:
                    self.visualize_reconstruction(
                        self.to_rgb(pred_img),
                        img_name=f"train/pred_x0",
                        add_step=True,
                        no_to_rgb=True,
                    )
        ...
        if self.global_step % self.train_cfg.log.log_every == 0:
            _log_tok_losses = self.format_log(train_out["log_mf_losses"])

            self.log_msg(
                f"[Train State]: lr {self.flow_optim.param_groups[0]['lr']:1.4e} | "
    ...
    def format_log(self, log_mf_loss: dict) -> str:
        ...
        strings = dict_round_to_list_str(log_mf_loss, select=list(log_mf_loss.keys()))
    ...
    def val_step(self, batch: dict):
        ...
        # sample using flow network
        with self.accelerator.autocast():
            sampled = self.mean_flow.sample(
                self.flow_model,
                bs=hrms_latent.shape[0],
                cond=condition,
                sample_steps=5,
                img_size=hrms_latent.shape[-2],
            )
            sampled_img = self.forward_tokenizer(sampled, mode="decode")["recon"]

        return {"sampled_img": sampled_img, "sampled_latent": sampled}
    ...
    def val_loop(self):
        ...
        for batch_or_idx in tbar:
            if self.val_cfg.max_val_iters <= 0:
                batch: dict = batch_or_idx
            else:
                batch: dict = next(self._val_loader_iter)

            val_out = self.val_step(batch)

            sample_img_rgb = self.to_rgb(val_out["sampled_img"])
...
def main(cfg):
    ...
    with catcher():
        trainer = MeanFlowGenerativeTrainer(cfg)
        trainer.run()
