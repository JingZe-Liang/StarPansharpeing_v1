...
def generate_all_golay_codewords(G):
    ...
    inputs = np.unpackbits(
        np.arange(n_words, dtype='>u2').view(np.uint8)
    ).reshape(n_words, 16)[:, -12:].astype(np.int32) 
...
def generate_leech_lattice_vectors():
    ...
    raw_signs = np.unpackbits(
        np.arange(256, dtype='>u2').view(np.uint8)
    ).reshape(256, 16)[:, -8:].astype(np.float32)
