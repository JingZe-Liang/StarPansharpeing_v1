...
configure_logger(
    sink="data/Downstreams/5Billion-ChinaCity-Segmentation/run_clip_patches.log",
    level="debug",
    _auto_=False,
    removed=True,
...
def postprocess_img(
    ...
    # defaults
    quantile, clip_mode = 0.999, "min"
    if "," in rescale:
        rescale, quantile, clip_mode = rescale.split(",")
    ...
    if normalize:
        ...
        if rescale == "clamp":
            ...
            img = per_channel_normalize(img, c_dim=(-2, -1) if img.ndim == 4 else (0, 1), quantile=quantile)
...
def img_saver_backend_compact_with_wds(
    ...
    if extension == "npy":
        ...
        np.savez_compressed(byte_io, img=img)  # img as key
    elif extension == "tiff":
        ...
        byte = safetensors.torch.save(  # uncompressed
            {"img": torch.as_tensor(img, dtype=getattr(torch, img.dtype.name))}
        )
        return byte
    elif extension == "webdataset":
        # left for webdataset to encode, usually npy format
        return img
    else:
...
def clip_img_to_webdataset(
    sink: wds.ShardWriter | wds.TarWriter | BinaryWriter | Cache,
    img_path: str | Path,
    ...
    def slide_image_and_save(img, img_name):
        ...
        for patch_idx, (patch, coord) in enumerate(slide_g):
            ...
            if dataset_type == "webdataset":
                ...
                sink._add_item(total_img_saved_count, data)
            else:
        ...
        return n_patches, img.shape, patch.shape, saved_name

    # img_name = Path(img_path).stem
    # tmp code: add parent dir name + stem
    img_name = Path(img_path).parent.stem + "_" + Path(img_path).stem
    ...
    if Path(img_path).suffix.lower() in [".mp4"] and isinstance(img, list):
        ...
        for i, img_f in enumerate(img):
            ...
            n_patch, shape, p_shape, saved_name = slide_image_and_save(img_f, img_name_frame)
    else:  # is image
        n_patch, shape, p_shape, saved_name = slide_image_and_save(img, img_name)

    return n_patch, shape, p_shape, saved_name
...
def loop_dataset_tif_MSI_images_to_webdataset(
    ...
    msi_files: str | Path | Iterable[str | Path] | None = None,
    img_clip_size: tuple[int, int] = (512, 512),
    img_stride: tuple[int, int] = (512, 512),
    img_resize: int | None = None,
    process_img_type: Literal["clip", "resize", "clip_resize", None] = "clip",
    ...
    if dataset_root is not None:
        ...
        for p in dataset_root:
            ...
            for glob_p in glob_pattern:
                collected_files.extend(p.glob(glob_p))
        msi_files_iter: Iterable[Path] = collected_files
    elif msi_files is not None:
        if isinstance(msi_files, (str, Path)):
            msi_files_iter = [Path(msi_files)]
        elif isinstance(msi_files, Iterable):
            msi_files_iter = msi_files
        else:
    ...
    if sort_msi_files:
        msi_files = natsort.natsorted(Path(msi_f) for msi_f in msi_files_iter)
        assert len(msi_files) > 0, "no MSI files found"
        logger.info(f"found {len(msi_files)} MSI files (sorted)")
    else:
        msi_files_iter = iter(msi_files_iter)
        first_file = next(msi_files_iter, None)
        assert first_file is not None, "no MSI files found"
        msi_files = itertools.chain([first_file], msi_files_iter)
        logger.info("found MSI files (streaming mode, count unknown)")
    ...
    def _runner(sink):
        ...
        for msi_file in (tbar := tqdm(msi_files, disable=not tqdm_or_not)):
            msi_file = Path(msi_file)
            n_patches, shape, patch_shape, saved_name = clip_img_to_webdataset(
                sink,
                img_path=msi_file,
                **run_main_kwargs,
            )
            ...
            if tqdm_or_not:
                tbar.set_description(f"writing {saved_name}, {n_patches=}, {shape=}, {patch_shape=}, {delete_file=}")
    ...
    if dataset_type == "webdataset":
        ...
        writer = Cache(input_dir=save_pattern_or_path, chunk_bytes="512Mb")
        _runner(writer)
    else:
...
def collect_tif_files_with_walk(dataset_root: str | Path, verbose=False) -> Iterator[Path]:
    from tqdm import tqdm

    root = Path(dataset_root)

    tbar = root.walk() if not verbose else tqdm(root.walk(), total=1_000_000)
    for dirpath, _, filenames in tbar:
...
def _read_bytes(path):
    path = Path(path)
    b = path.read_bytes()
    name = path.parent.stem + "_" + path.stem
    return {"__key__": name, "img": b}


def fast_loop_read_into_litdata(
    dataset_root="data2/SSL4EO-L/etm_sr/ssl4eo_l_etm_sr", output_dir="data2/SSL4EO-L/etm_sr/etm_sr_litdata"
):
    paths_iter = collect_tif_files_with_walk(dataset_root, True)
...
if __name__ == "__main__":
    ...
    # set_logger_file("data2/SSL4EO-L/etm_sr/etm_sr_litdata.log", "debug", add_time=True)

    # func_kwargs = {
    #     "process_img_type": "resize",
    #     "img_clip_size": (264, 264),
    #     "img_stride": (264, 264),
    #     "img_resize": 264,
    #     "img_rescale": "clamp",
    #     "save_backend": "tiff",
    ...
    # dataset_path = "data2/SSL4EO-L/etm_sr/etm_sr_litdata"
    # all_msi_files_s = collect_tif_files_with_walk("data2/SSL4EO-L/etm_sr/ssl4eo_l_etm_sr")
    # # logger.info(f"Found {len(all_msi_files_s)} MSI files for processing.")
    # # for p in all_msi_files_s:
    # #     print(p)

    # loop_dataset_tif_MSI_images_to_webdataset(
    #     save_pattern_or_path=dataset_path,
    #     msi_files=all_msi_files_s,
    #     **func_kwargs,
    # )
