    ...
    if normalize:
        ...
        if rescale == "clamp":
            ...
            img = per_channel_normalize(
                img, c_dim=(-2, -1) if img.ndim == 4 else (0, 1), quantile=quantile
            )
...
def clip_img_to_webdataset(
    ...
    if Path(img_path).suffix.lower() in [".mp4"] and isinstance(img, list):
        ...
        for i, img_f in enumerate(img):
            ...
            n_patch, shape, p_shape, saved_name = slide_image_and_save(
                img_f, img_name_frame
            )
    else:  # is image
        ...
        for msi_file in (tbar := tqdm(msi_files, disable=not tqdm_or_not)):
            ...
            if tqdm_or_not:
                tbar.set_description(
                    f"writing {saved_name}, {n_patches=}, {shape=}, {patch_shape=}, {delete_file=}"
                )
