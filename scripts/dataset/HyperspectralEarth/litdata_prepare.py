...
configure_logger(sink="tmp/run.log", level="debug", _auto_=False, removed=True, add_tqdm_filter=True)
...
if __name__ == "__main__":
    ...
    set_logger_file("tmp/dynamic_earth_webdataset.log", "debug", add_time=False)
    ...
    if not _mp:
        # SpectralEarth Dataset, 1024px original, clip into 128px
        path = "/Data2/ZihanCao_tmp_files/dataset"
        logger.info("Collect images ...")
        # all_msi_files_s = list(Path(path).rglob("*.tif"))
        ...
        # )
        webdataset_pts = "data2/HyperspectralEarth/LitData_hyper_images"

        def _walk_dir_with_tif(n: int = 400, dir_path: str = path):
            batch = []
            n_batches = 0
            for root, dirs, files in Path(dir_path).walk():
                for file in files:
                    if file.lower().endswith((".tif", ".tiff")):
                        batch.append(str(Path(root) / file))
                        if len(batch) >= n:
                            logger.info(
                                f"Yielding batch of {len(batch)} files.",
                                n_batches=n_batches,
                            )
                            yield batch
                            n_batches += 1
        ...
        for all_msi_files_s in _walk_dir_with_tif(n=400, dir_path=path):
            loop_dataset_tif_MSI_images_to_webdataset(
                save_pattern_or_path=webdataset_pts,
        ...
        #     # split n list
        #     n = len(all_msi_files_s)
        #     # 400 parts
        #     all_msi_files_lst = []
        #     for i in range(0, n, 400):
        #         all_msi_files_lst.append(all_msi_files_s[i : i + 400])
        #     all_msi_files_s = all_msi_files_lst

        # if not isinstance(webdataset_pts, list):
        #     webdataset_pts = [webdataset_pts] * len(all_msi_files_s)
